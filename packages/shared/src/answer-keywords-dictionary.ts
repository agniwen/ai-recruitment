...
/**
 * 带含义的数字/绩效。四类分支（多字单位 `万元` 必须排在单字 `万`/`元` 之前）：
 * 1. 货币符号前缀数字：`￥500`、`￥500万`
 * 2. 阿拉伯数字紧跟单位/百分号：`30%`、`10人`、`3.5年`、`500万元`
 * 3. 中文数字/量级 + 万/亿：`千万`、`二十多万`、`上百万`、`几亿`（覆盖中文口语量级）
 * 4. 字母等级评价：`S级`、`A级`、`B+级`（绩效评级，归 metric）
 * 已知噪声（接受）：`2024年`（含单位年份）、`第3名`（序数排名）、`千万`（也可作副词「千万别」）
 * 均会被当 metric 命中——规则不做左侧上下文判定，属可接受噪声。
 */
export const METRIC_REGEX =
  /[￥¥]\d+(?:\.\d+)?(?:[万亿千])?(?:元)?|\d+(?:\.\d+)?(?:万元|万|亿|千|个|人|名|位|年|月|天|次|倍|条|项|台|件|元|%|％)|[零一二三四五六七八九十百千两几上数多]+[万亿](?:元)?|[SABCDEFsabcdef][+-]?级/g;

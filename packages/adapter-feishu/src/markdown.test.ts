...
describe("feishuFormatConverter", () => {
  ...
  describe("fromAst (AST -> Feishu markdown)", () => {
    ...
    it("should convert @mentions with mixed special chars", () => {
      ...
      const result = converter.fromAst(ast);
      expect(result).toContain('<at user_id="first.last-name">first.last-name</at>');
    });
  });
  ...
  describe("toAst (Feishu markdown -> AST)", () => {
    ...
    it("should convert Feishu user mentions to standard @mentions", () => {
      const ast = converter.toAst('Hello <at user_id="ou_xxx">Alice</at> how are you?');
    ...
    it("should handle multiple Feishu mentions", () => {
      const ast = converter.toAst('<at user_id="ou_1">Alice</at> and <at user_id="ou_2">Bob</at>');
      const roundTripped = converter.fromAst(ast);
      expect(roundTripped).toContain("Alice");
  ...
  describe("extractPlainText", () => {
    ...
    it("should extract link text", () => {
      expect(converter.extractPlainText("Check [this](https://example.com)")).toBe("Check this");
    });

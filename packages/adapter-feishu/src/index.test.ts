...
function unsetEnv(
  key: "FEISHU_APP_ID" | "FEISHU_APP_SECRET" | "FEISHU_ENCRYPT_KEY" | "FEISHU_VERIFICATION_TOKEN",
): void {
...
function restoreFeishuEnv(): void {
  ...
  if (originalEnv.FEISHU_VERIFICATION_TOKEN === undefined) {
    ...
    process.env.FEISHU_VERIFICATION_TOKEN = originalEnv.FEISHU_VERIFICATION_TOKEN;
  }
}
...
function createMessageEventPayload(overrides?: { token?: string; senderType?: "user" | "app" }): {
  schema: string;
  header: {
    app_id: string;
    create_time: string;
...
describe("createFeishuAdapter", () => {
  ...
  it("reads encryptKey and verificationToken from config", () => {
    ...
    expect(Reflect.get(adapter, "verificationToken")).toBe("config-verification-token");
  });
  ...
  it("reads encryptKey and verificationToken from env vars", () => {
    ...
    expect(Reflect.get(adapter, "verificationToken")).toBe("env-verification-token");
  });
});
...
describe("decodeThreadId", () => {
  ...
  it("throws ValidationError for invalid format", () => {
    ...
    expect(() => adapter.decodeThreadId("feishu:oc_xxx")).toThrow(ValidationError);
    expect(() => adapter.decodeThreadId("slack:oc_xxx:om_xxx")).toThrow(ValidationError);
  });
});
...
describe("channelIdFromThreadId", () => {
  ...
  it("returns feishu:chatId", () => {
    expect(adapter.channelIdFromThreadId("feishu:oc_xxx:om_xxx")).toBe("feishu:oc_xxx");
  });
});
...
describe("handleWebhook", () => {
  ...
  it("returns challenge response for URL verification", async () => {
    const adapter = createTestAdapter();
    const request = createWebhookRequest(JSON.stringify({ challenge: "test-challenge-token" }));
...
describe("handleWebhook - signature verification", () => {
  ...
  it("uses timing-safe comparison for signature verification", async () => {
    ...
    const validSignature = crypto.createHash("sha256").update(content).digest("hex");
...
describe("parseMessage", () => {
  ...
  it("sets edited metadata from update_time", () => {
    ...
    expect(message.metadata?.editedAt).toEqual(new Date(1_700_000_060_000));
  });
...
describe("startTyping", () => {
  ...
  it("is a no-op and does not throw", async () => {
    await expect(adapter.startTyping("feishu:oc_xxx:om_xxx")).resolves.toBeUndefined();
  });
});

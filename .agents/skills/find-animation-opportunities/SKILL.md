...
Every "Suggested motion" cell carries exact values — the curve, the duration, the properties — pulled from the audited codebase's existing motion conventions. In this repository, reuse the inline Tailwind easing and duration patterns already present under `apps/ai-recruitment-copilot` instead of inventing a parallel token layer. When no local value exists, use these references without approximation: `cubic-bezier(0.23, 1, 0.32, 1)` for ease-out, `cubic-bezier(0.77, 0, 0.175, 1)` for ease-in-out, and `cubic-bezier(0.32, 0.72, 0, 1)` for drawers. Animate `transform` and `opacity` only; include reduced-motion handling (gentler, not zero) and `@media (hover: hover) and (pointer: fine)` gating when the suggestion involves hover.

...
- This monorepo keeps most motion values close to components, for example in `apps/ai-recruitment-copilot/src/components/ui/button.tsx`; follow the owning package's existing Tailwind pattern instead of inventing a root `src/styles/tokens.css`.

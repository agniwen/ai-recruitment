...
describe("TransitionCandidateDialog reactivation", () => {
  it.each(["screening", "ai_review"] as const)(
    "reactivates candidates closed in %s without bypassing approval",
    async (previousStage) => {
      apiMocks.fetchStudioResume.mockResolvedValue({ closedMeta: { previousStage } });
      const container = document.createElement("div");
      document.body.append(container);
      const root = createRoot(container);
      const queryClient = new QueryClient({

...
// oxlint-disable-next-line eslint/complexity -- resume edit dialog coordinates fetching, fork-only hiring-unit selection, and validated updates.
function ResumeEditBody({
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      // 编辑弹窗不再提交简历文件 / 简历评价（notes / resumeReview），服务端也会忽略这些字段。
      // Edit no longer submits resume files or resume notes/reviews; the backend ignores them too.
      ...
      formData.append("hiringUnitId", value.hiringUnitId ?? "");
    ...
    <Modal
      footer={
        <>
          ...
          <Button
            disabled={isSubmitting || query.isLoading || isResumeLocked || hiringUnitInvalid}
...
function InterviewEditBody({
  ...
  return (
    <>
      <Modal
        description="编辑轮次排期、文本输入设置和备注。状态由系统流转，只读展示；候选人基础信息请在候选人管理编辑。"
        footer={
          isLoading ? undefined : (
            ...
            <Card className="gap-0 rounded-lg py-0">
              <CardContent className="flex flex-col gap-3 bg-muted/20 px-3 py-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-muted-foreground text-sm leading-normal">
                  候选人身份字段、关联岗位、简历和简历评价统一在候选人管理维护。
                </p>
                {round?.candidate.id ? (
                  <Button
                    className="shrink-0"

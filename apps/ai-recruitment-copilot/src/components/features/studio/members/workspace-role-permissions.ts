...
const PAGE_PERMISSION_DESCRIPTIONS: Partial<Record<string, string>> = {
  calendar:
    "控制是否能在侧边栏看到并访问「面试日程」页面；未勾选时侧边栏不展示入口，直接访问会进入 404。用于按日历查看 AI 面试和真人面试安排。日程列表与 AI 面试事件预览接口也受该页面权限控制。日历事件 hovercard 上的「查看候选人 / 查看面试」仍分别受「候选人管理」「AI 面试管理」页面权限控制。",
  ...
  forms:
    "控制是否能在侧边栏看到并访问「AI面试-面前通用题」页面；用于设置 AI 面试前的通用题库。页面内列表、AI 生成、版本和增删改仍受「表单题」相关权限控制。",
  globalConfig:
    "控制是否能在侧边栏看到并访问「公司信息与话术」页面；用于设置公司信息、岗位编码及 AI 面试开场和结束话术。读取和保存仍受「上下文设置」查看/编辑权限控制。",
  interviewQuestions:
    "控制是否能在侧边栏看到并访问「AI 面试-沟通通用题」页面；用于设置 AI 面试过程中的提问及追问题目通用模板，也会配合简历解析生成的沟通题使用。题库列表、版本和增删改仍受「沟通题」相关权限控制。",
  interviewers:
    "控制是否能在侧边栏看到并访问「AI 面试官」页面；用于配置 AI 面试官的人设、追问风格和声音。面试官列表、详情和增删改仍受「面试官」相关权限控制。",
  interviews:
    "控制是否能在侧边栏看到并访问「AI 面试管理」页面；用于查看候选人的 AI 面试轮次、进度和面试报告。列表、详情、报告、录音、轮次和操作仍受「AI 面试」相关权限控制。面试日程 hovercard 的「查看面试」也依赖该权限。",
  jobDescriptions:
  ...
  resumePool:
    "控制是否能在侧边栏看到并访问「简历池」页面；用于收集和查看尚未进入正式招聘流程的公共、私有简历。未勾选时直接访问会进入 404。页面内数据接口仍受「公共简历池」业务权限控制。",
  resumes:
    "控制是否能在侧边栏看到并访问「候选人管理」页面；用于管理已正式入库候选人的筛选、面试、Offer 及结案进度。未勾选时直接访问会进入 404。候选人数据接口仍受「简历库」业务权限控制，上传批次有独立权限。面试日程 hovercard 的「查看候选人」也依赖该权限。",
};

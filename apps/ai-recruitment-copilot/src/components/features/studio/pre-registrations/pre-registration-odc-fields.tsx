...
export function PreRegistrationOdcFields({
  ...
  scopeMode?: OdcScopeMode;
  onScopeModeChange?: (mode: OdcScopeMode) => void;
}) {
  ...
  return (
    <FieldGroup>
      {onScopeModeChange ? (
        <Field>
          <FieldLabel htmlFor="odc-scope-mode">ODC 负责范围</FieldLabel>
          <Select
            value={scopeMode}
            disabled={disabled}
            onValueChange={(value) => {
          ...
          >
            <SelectTrigger id="odc-scope-mode" className="w-full">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
                <SelectItem value="all">全部部门/中心（包含以后新增）</SelectItem>
                <SelectItem value="selected">指定部门/中心</SelectItem>
              </SelectGroup>
            </SelectContent>
          </Select>
          <FieldDescription>
            {scopeMode === "all"
              ? "负责当前工作区全部部门/中心，包含以后新增；不限序列和服务单位。"
              : "仅负责所选部门/中心；未选择时，不获得 ODC 来源授权。"}
          </FieldDescription>
        </Field>
      ) : null}
      {scopeMode === "selected" ? (
        <>
          <Field>
            ...
            <SearchableMultiSelect
              ...
              placeholder={
                loading ? "加载部门/中心（来源）中…" : "请选择部门/中心（来源）（可多选）"
              }
              searchPlaceholder="搜索部门/中心（来源）"
              emptyMessage="暂无可选部门/中心（来源）"
            />
            <FieldDescription>
              可分别设置每个来源的序列和服务单位；这两项留空表示不限。
            </FieldDescription>
            {failed ? (

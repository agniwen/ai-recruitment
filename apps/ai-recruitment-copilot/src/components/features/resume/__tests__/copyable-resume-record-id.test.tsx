...
describe("CopyableResumeRecordId", () => {
  it("renders the masked id and a dedicated copy control", () => {
    const html = renderToStaticMarkup(<CopyableResumeRecordId id="sdvs1234xscs" parentheses />);
    expect(html).toContain("(sdvs****xscs)");
    expect(html).toContain('aria-label="复制简历 ID"');
  ...
  it("copies via clipboard helper and stops event bubbling", () => {
    ...
    expect(source).toContain('toast.success("已复制简历 ID")');

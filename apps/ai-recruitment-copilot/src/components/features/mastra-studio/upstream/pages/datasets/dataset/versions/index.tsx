...
function DatasetCompareVersionsPage() {
  ...
  const allItems = useMemo(() => {
    ...
    for (const item of [...itemsA, ...itemsB]) {
      if (!seen.has(item.id)) {
        seen.set(item.id, { createdAt: new Date(item.createdAt), id: item.id });
      }
    }
    return [...seen.values()].toSorted((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
  }, [itemsA, itemsB]);
  ...
  const handleItemClick = (
    itemId: string,
    itemA?: { datasetVersion: number },
    itemB?: { datasetVersion: number },
  ) => {
    void navigate(

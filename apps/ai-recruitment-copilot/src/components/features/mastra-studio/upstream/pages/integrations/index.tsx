...
import { useAuthorize } from "@/components/features/mastra-studio/upstream/domains/tool-providers/hooks/use-authorize";
import { useDisconnectConnection } from "@/components/features/mastra-studio/upstream/domains/tool-providers/hooks/use-disconnect-connection";
import { useExistingConnections } from "@/components/features/mastra-studio/upstream/domains/tool-providers/hooks/use-existing-connections";
import { useIsToolProviderAdmin } from "@/components/features/mastra-studio/upstream/domains/tool-providers/hooks/use-is-tool-provider-admin";
import { useToolProviders } from "@/components/features/mastra-studio/upstream/domains/tool-providers/hooks/use-tool-providers";
import { useToolkits } from "@/components/features/mastra-studio/upstream/domains/tool-providers/hooks/use-toolkits";
...
export default function IntegrationsPage() {
  ...
  const groupedByAuthor = useMemo(
    () => getGroupedConnectionsByAuthor(connections, isAdmin),
    [connections, isAdmin],
  );
  ...
  const handleConnect = () => {
    ...
    authorize.mutate(
      { label: label.trim() || null, providerId, toolkit },
      {
        onSuccess: () => {
          setLabel("");
  ...
  const handleDisconnect = (connectionId: string) => {
    disconnect.mutate({ connectionId, providerId });
  };

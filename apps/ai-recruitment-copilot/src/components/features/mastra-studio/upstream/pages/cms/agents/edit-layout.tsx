...
import { getCodeAgentOverrideSections } from "@/components/features/mastra-studio/upstream/domains/agents/components/agent-cms-sidebar/agent-cms-sections";
import { AgentVersionPanel } from "@/components/features/mastra-studio/upstream/domains/agents/components/agent-version-panel";
...
function EditLayoutWrapper() {
  ...
  useEffect(() => {
    if (!isCodeAgentOverride || codeAgentOverrideSections.length === 0) {
      return;
    }

    const isAllowedPath = codeAgentOverrideSections.some(

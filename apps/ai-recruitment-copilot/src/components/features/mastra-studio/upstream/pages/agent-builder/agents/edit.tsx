...
import {
  ...
  AgentProfileHero,
  AgentProfileTabs,
} from "@/components/features/mastra-studio/upstream/domains/agent-builder/components/agent-edit/agent-profile";
import { AutosaveIndicator } from "@/components/features/mastra-studio/upstream/domains/agent-builder/components/agent-edit/autosave-indicator";
import { ConversationPanelChat } from "@/components/features/mastra-studio/upstream/domains/agent-builder/components/agent-edit/conversation-panel";
import { DeleteAgentPanelButton } from "@/components/features/mastra-studio/upstream/domains/agent-builder/components/agent-edit/delete-agent-action";
import { EditTopBar } from "@/components/features/mastra-studio/upstream/domains/agent-builder/components/agent-edit/edit-top-bar";
import { VisibilitySelect } from "@/components/features/mastra-studio/upstream/domains/agent-builder/components/agent-edit/visibility-select";
...
import {
  ...
  useAgentPrimitives,
} from "@/components/features/mastra-studio/upstream/domains/agent-builder/contexts/agent-primitives-context";
import {
  EditPageProvider,
  useEditPage,
} from "@/components/features/mastra-studio/upstream/domains/agent-builder/contexts/edit-page-context";
import {
  useStreamRunning,
  useStreamRunningDebounced,
} from "@/components/features/mastra-studio/upstream/domains/agent-builder/contexts/stream-chat-context";
import {
  useWizard,
  WizardProvider,
} from "@/components/features/mastra-studio/upstream/domains/agent-builder/contexts/wizard-context";
import { useAvailableAgentTools } from "@/components/features/mastra-studio/upstream/domains/agent-builder/hooks/use-available-agent-tools";
...
import { AgentBuilderEditLayout } from "@/components/features/mastra-studio/upstream/domains/agent-builder/layouts/agent-builder-edit-layout";
...
import { useAllProviderTools } from "@/components/features/mastra-studio/upstream/domains/tool-providers/hooks/use-all-provider-tools";
import { startViewTransition } from "@/components/features/mastra-studio/upstream/lib/routing";

const isFilled = (isDirty: boolean | undefined, value: string) =>
  Boolean(isDirty) || value.trim().length > 0;
...
const ProfileSlot = () => {
  ...
  const heroActions = (
    <div
      ...
      data-testid="agent-builder-hero-actions-desktop"
    >
      ...
      {isOwner && (
        <DeleteAgentPanelButton agentId={agentId} agentName={name} disabled={isRunning} />
      )}
    </div>
  );
...
const EditPageLayout = () => {
  ...
  const shouldBeCentered =
    (step === "ready" && !isBuilderReady) || (step === "identity" && !hasMandatoryFields);
...
const EditPageGate = () => {
  ...
  if (!isReady || !storedAgent) {
    return <AgentBuilderAgentEditSkeleton />;
  }
  if (!storedAgent) {
    return <Navigate to="/agent-builder/agents" replace />;
  }
  if (!canWrite || !isOwner) {
    return <Navigate to={`/agent-builder/agents/${agentId}/view`} replace />;
  }

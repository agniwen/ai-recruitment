...
export default function LogsPage() {
  ...
  const { logIdMap, featuredLog, handleLogClick, handlePreviousLog, handleNextLog } =
    useLogsListNavigation(logs, url.featuredLogId, url.handleFeaturedChange, url.featuredTraceId);

  const { data: lightSpansData, isLoading: isLoadingLightSpans } = useTraceLightSpans(
    url.featuredTraceId ?? null,
  );
  ...
  const handleTraceClick = useCallback(
    (traceId: string) => url.handleFeaturedChange({ spanId: null, traceId }),
    [url],
  );
  const handleSpanClick = useCallback(
    (traceId: string, spanId: string) => url.handleFeaturedChange({ spanId, traceId }),
    [url],
  );
  const handleTraceClose = useCallback(() => {
    url.handleFeaturedChange({ spanId: null, traceId: null });

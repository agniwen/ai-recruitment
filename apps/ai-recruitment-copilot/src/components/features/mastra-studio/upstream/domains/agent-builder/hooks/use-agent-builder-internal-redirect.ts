...
export function useAgentBuilderInternalRedirect() {
  const { data: draftAgentsData, isLoading: isLoadingDraftAgents } = useStoredAgents({
    status: "draft",
  });
  const { data: publishedAgentsData, isLoading: isLoadingPublishedAgents } = useStoredAgents({
    status: "published",
  });

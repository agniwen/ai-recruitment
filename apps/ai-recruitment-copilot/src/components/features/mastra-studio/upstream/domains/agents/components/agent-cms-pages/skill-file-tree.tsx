...
function containsNode(nodes: InMemoryFileNode[], targetId: string): boolean {
  for (const node of nodes) {
    ...
    if (node.children && containsNode(node.children, targetId)) {
      return true;
    }
  }
  return false;
      ...
      if (node.type === "folder") {
        ...
        return (
          <Tree.Folder
            key={node.id}
            open={isOpen}
            onOpenChange={(open: boolean) => onFolderOpenChange(node.id, open)}
          >
            <Tree.FolderTrigger
              actions={
                !readOnly && (
    ...
    (name: string) => {
      ...
      const newNode: InMemoryFileNode =
        pendingInput.type === "folder"
          ? { children: [], id: newId, name, type: "folder" }
          : { content: "", id: newId, name, type: "file" };

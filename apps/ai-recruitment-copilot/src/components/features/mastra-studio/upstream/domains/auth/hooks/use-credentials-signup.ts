...
export interface CredentialsSignUpRequest {
  email: string;
  password: string;
  name?: string;
}

export interface CredentialsSignUpResponse {
  user?: {
    id: string;
    email: string;
    name?: string;
 ...
 * @example
 * ```tsx
 * import { useCredentialsSignUp } from '@/components/features/mastra-studio/upstream/domains/auth/hooks/use-credentials-signup';
 *
 * function SignUpForm() {
...
export function useCredentialsSignUp() {
  ...
  return useMutation<CredentialsSignUpResponse, Error, CredentialsSignUpRequest>({
    mutationFn: ({ email, password, name }) =>
      makeCredentialsSignUpRequest(client, { email, name, password }),
    onSuccess: () => {

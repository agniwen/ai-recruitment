...
import { Navigate, useLocation } from "@/components/features/mastra-studio/router/compat";
...
export function RoutePermissionGuard({ children }: { children: React.ReactNode }) {
  ...
  const { hasPermission, hasAnyPermission, rbacEnabled, isAuthenticated, isLoading } =
    usePermissions();

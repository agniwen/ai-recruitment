...
export function ExperimentalUIManager({ pathname }: { pathname?: string }) {
  ...
  const visibleExperiments = pathname
    ? experiments.filter(
        (e) => !e.path || (Array.isArray(e.path) ? e.path.includes(pathname) : e.path === pathname),
      )
    : experiments;
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button aria-label="实验性界面" size="sm" className="mr-auto ml-3 bg-blue-600 text-white">
          <FlaskConicalIcon /> 实验性界面
        </Button>
      </PopoverTrigger>

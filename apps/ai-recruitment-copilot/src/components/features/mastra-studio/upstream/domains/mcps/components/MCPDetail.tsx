...
import { ToolIconMap } from "@/components/features/mastra-studio/upstream/domains/tools";
...
export const MCPDetail = ({ isLoading, server }: MCPDetailProps) => {
  ...
  useEffect(() => {
    ...
    const sseUrl = getEmbeddedMastraApiUrl(`/mcp/${server.id}/sse`);
    const httpStreamUrl = getEmbeddedMastraApiUrl(`/mcp/${server.id}/mcp`);

    setUrls({ httpStreamUrl, sseUrl });
  }, [server]);

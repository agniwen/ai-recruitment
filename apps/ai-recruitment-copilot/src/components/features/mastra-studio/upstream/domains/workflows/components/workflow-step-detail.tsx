    ...
    <div className="flex flex-col h-full" data-testid="workflow-step-detail-panel">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border1 bg-surface1">
        <div className="flex items-center gap-2">
          {stepDetail.type === "map-config" && (
            <List className="w-4 h-4" style={{ color: BADGE_COLORS.map }} />
          )}
          ...
          <div className="flex flex-col">
            <Txt variant="ui-md" className="text-neutral6 font-medium">
              {stepDetail.type === "map-config"
                ? `${stepDetail.stepName} Config`
                : `${stepDetail.stepName} Workflow`}
            </Txt>
            {stepDetail.type === "map-config" &&
              stepDetail.stepId &&
              stepDetail.stepId !== stepDetail.stepName && (
      ...
      <div className="flex-1 overflow-auto">
        {stepDetail.type === "map-config" && stepDetail.mapConfig && (
          <CodeDialogContent data={stepDetail.mapConfig} />
        )}

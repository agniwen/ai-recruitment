...
export const useDatasetMutations = () => {
  ...
  const updateItem = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      void queryClient.invalidateQueries({
        queryKey: ["dataset-item", variables.datasetId, variables.itemId],
      });
      void queryClient.invalidateQueries({
  ...
  const triggerExperiment = useMutation({
    ...
    onSuccess: (_, variables) => {
      void queryClient.invalidateQueries({
        queryKey: ["dataset-experiments", variables.datasetId],
      });
    },
  });
  ...
  const updateExperimentResult = useMutation({
    mutationFn: (params: UpdateExperimentResultParams) =>
      client.updateDatasetExperimentResult(params),
    onSuccess: (_, variables) => {
      void queryClient.invalidateQueries({
        queryKey: ["experiment-results", variables.experimentId],
      });

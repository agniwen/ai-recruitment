...
export function BrowserToolCallsProvider({ children }: { children: ReactNode }) {
  ...
  const toolCalls = useMemo(
    () => [...toolCallMap.values()].toSorted((a, b) => a.timestamp - b.timestamp),
    [toolCallMap],
  );

  const value = useMemo(() => ({ registerToolCall, toolCalls }), [toolCalls, registerToolCall]);

  return (
    <BrowserToolCallsContext.Provider value={value}>{children}</BrowserToolCallsContext.Provider>
  );
}

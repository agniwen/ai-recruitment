      ...
      <TooltipTrigger asChild>
        <span
          className={cn(
            ...
            enabled
              ? "border-border1 bg-surface4 text-neutral6"
              : "border-border1/40 text-neutral3/50",
          )}
        >
...
function MemorySidebarSkeleton() {
  return (
    <div
      data-testid="memory-sidebar-skeleton"
      className="flex h-full min-h-0 w-full flex-col gap-2.5 p-3"
    >
...
function MemorySidebarBody({ agentId, threadId, threads, onDelete }: MemorySidebarProps) {
  ...
  const { lastMessages, semanticRecallOn, workingMemoryOn, observationalOn } =
    useMemoryFeatureFlags(agentId);

...
interface ExperimentTraceTimelineTimingColProps {
...
export function ExperimentTraceTimelineTimingCol({
  ...
  const percentageSpanLatency = overallLatency
    ? Math.ceil((span.latency / overallLatency) * 100)
    : 0;
  ...
  const spanStartTimeShift =
    spanStartTimeDate && overallStartTimeDate
      ? spanStartTimeDate.getTime() - overallStartTimeDate.getTime()
      : 0;

  const percentageSpanStartTime =
    overallLatency && Math.floor((spanStartTimeShift / overallLatency) * 100);
  ...
  return (
    <HoverCard.Root openDelay={250}>
      ...
      >
        <div
          className={cn(
            "w-full p-2.5 rounded-lg bg-surface4 transition-colors duration-1000 min-w-40",
          )}
        >
        ...
        <div className={cn("flex justify-end text-neutral3 text-ui-sm")}>

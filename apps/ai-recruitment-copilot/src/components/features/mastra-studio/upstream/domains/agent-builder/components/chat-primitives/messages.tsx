...
} from "@/components/features/mastra-studio/upstream/lib/ai-ui/messages/signal-data";
...
const toReactiveSignalMessage = (message: MastraDBMessage): MastraDBMessage | null => {
  ...
  return {
    ...
    content: {
      ...message.content,
      parts: [{ data, type: "data-signal" }],
    },
    role: "assistant",
  } as MastraDBMessage;
};

const getRequireApprovalMetadata = (
  message: MastraDBMessage,
): RequireApprovalMetadata | undefined => {
  const metadata = message.content.metadata as MastraDBMessageMetadata | undefined;
  if (!metadata || typeof metadata !== "object") {
    return undefined;
  }
  const { mode } = metadata;
  if (mode !== "stream" && mode !== "network" && mode !== "generate") {
    return undefined;
  }
...
const findApprovalEntry = (
  ...
  return (
    (toolName ? approvals[toolName] : undefined) ?? (toolCallId ? approvals[toolCallId] : undefined)
  );
};
...
export const MessageRow = ({ message }: MessageRowProps) => {
  ...
  const renderers: MessageRenderers = {
    Data: (part) =>
      part.type === "data-signal" && isSignalData(part.data) ? (
        <SignalBadge signal={part.data} />
      ) : null,

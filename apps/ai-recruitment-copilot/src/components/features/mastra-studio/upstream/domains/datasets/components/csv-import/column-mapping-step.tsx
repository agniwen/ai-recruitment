...
import { usePortalContainer } from "@mastra/playground-ui/primitives/portal-container";
...
export function ColumnMappingStep({ headers, mapping, onMappingChange }: ColumnMappingStepProps) {
  const portalContainer = usePortalContainer() as HTMLElement | undefined;
  ...
  const getColumnsForZone = (zone: FieldType): string[] =>
    headers.filter((header) => mapping[header] === zone);

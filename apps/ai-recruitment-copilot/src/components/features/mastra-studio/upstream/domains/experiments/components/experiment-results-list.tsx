...
export interface ExperimentResultsListProps {
...
export function ExperimentResultsList({
  ...
  const gridColumns = [hasSelection ? "auto" : "", ...columns.map((c) => c.size)]
    .filter(Boolean)
    .join(" ");
          ...
          {results.map((result) => {
            ...
            const rowCells = (
              <>
                ...
                {hasInputColumn && (
                  <DataList.MonoCell>{truncate(formatValue(result.input), 200)}</DataList.MonoCell>
                )}

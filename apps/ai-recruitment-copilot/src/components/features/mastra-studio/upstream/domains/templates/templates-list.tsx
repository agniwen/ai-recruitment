  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={cn(
            "grid py-3 px-6 w-full gap-0.5",
            "[&_svg]:w-[1em] [&_svg]:h-[1em] [&_svg]:text-neutral3",
          )}
        >
          <h2 className="text-ui-lg text-neutral5">{template.title}</h2>
          <p className="text-ui-md text-neutral4 transition-colors duration-500">
  ...
  return (
    <div className={cn("grid gap-y-4", className)}>
      {templates.map((template) => (
        <TemplateCard key={template.slug} LinkComponent={LinkComponent} template={template} />
      ))}
    </div>
  );
}

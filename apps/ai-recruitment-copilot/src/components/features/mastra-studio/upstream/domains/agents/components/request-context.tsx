...
import type { RequestContextPresets } from "@/components/features/mastra-studio/upstream/domains/request-context/hooks/use-request-context-presets";
import { useRequestContextPresets } from "@/components/features/mastra-studio/upstream/domains/request-context/hooks/use-request-context-presets";
...
function getMatchingPresetKey(presets: RequestContextPresets | null, requestContextStr: string) {
  ...
  for (const [key, value] of Object.entries(presets)) {
    if (JSON.stringify(value) === requestContextStr) {
      return key;
    }
  }
...
export const RequestContext = ({
  ...
  const [selectedPreset, setSelectedPreset] = useState<string>(() =>
    getMatchingPresetKey(presets, requestContextStr),
  );
  ...
  const handlePresetChange = async (presetKey: string) => {
    ...
    if (presetKey === "__custom__" || !presets) {
      return;
    }
    ...
    <div>
      <Notice
        ...
        action={
          <Notice.Button
            as={Link}
            to="https://mastra.ai/docs/server/request-context"
            target="_blank"
          >
            <Icon>
              <ExternalLink />

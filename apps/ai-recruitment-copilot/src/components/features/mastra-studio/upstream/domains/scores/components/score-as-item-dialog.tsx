...
import { SaveAsDatasetItemDialog } from "@/components/features/mastra-studio/upstream/domains/datasets/components/save-as-dataset-item-dialog";

interface ScoreAsItemDialogProps {
  score?: ScoreRowData;
  isOpen: boolean;
  onClose: () => void;
...
function getInitialInput(score?: ScoreRowData): string {
  ...
  return JSON.stringify({ groundTruth: null, input: score.input, output: score.output }, null, 2);
}
...
function getInitialGroundTruth(score?: ScoreRowData): string {
  ...
  return JSON.stringify({ reason: score.reason ?? null, score: score.score }, null, 2);
}
...
export function ScoreAsItemDialog({ score, isOpen, onClose, level = 2 }: ScoreAsItemDialogProps) {
  return (
    <SaveAsDatasetItemDialog
      ...
      onClose={onClose}
      level={level}
      source={score?.traceId ? { referenceId: score.traceId, type: "trace" } : undefined}
    />
  );
}

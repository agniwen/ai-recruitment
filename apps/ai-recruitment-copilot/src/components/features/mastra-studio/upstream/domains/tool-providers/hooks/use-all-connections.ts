...
import {
  ...
  useCurrentUser,
} from "@/components/features/mastra-studio/upstream/domains/auth/hooks/use-current-user";
...
export const useAllConnections = (options?: UseAllConnectionsOptions) => {
  ...
  const hasConnection = useCallback(
    (providerId: string, toolkit: string) =>
      (connectionsByKey.get(`${providerId}:${toolkit}`) ?? []).some(
  ...
  return { getConnections, hasConnection, isLoading };
};

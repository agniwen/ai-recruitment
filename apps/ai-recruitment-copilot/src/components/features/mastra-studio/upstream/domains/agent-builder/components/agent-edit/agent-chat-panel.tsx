...
import { BrowserThumbnail } from "@/components/features/mastra-studio/upstream/domains/agents/components/browser-view";
...
export const AgentChatPanelProvider = ({
  ...
  const meta = useMemo<AgentChatMeta>(
    () => ({ agentAvatarUrl, agentDescription, agentName, isConversationLoading }),
...
const AgentChatMessageList = ({ onStarterPromptSelect }: AgentChatMessageListProps) => {
  ...
  const { isConversationLoading, agentName, agentDescription, agentAvatarUrl } =
          ...
          <div className="grid w-full max-w-2xl grid-cols-1 gap-5 sm:grid-cols-2">
            {STARTER_PROMPTS.map((starterPrompt, index) => (
              <button
                ...
                data-testid={`agent-builder-agent-chat-starter-${starterPrompt.title.toLowerCase().replaceAll(/\s+/g, "-")}`}
              ...
              >
                ...
                <span className="min-w-0">
                  ...
                  <Txt
                    variant="ui-xs"
                    className="mt-1 text-neutral4 transition-colors group-hover:text-neutral5"
                  >

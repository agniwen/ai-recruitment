...
export type ReviewByExperiment = Map<
  string,
  { needsReview: number; complete: number; total: number }
>;
...
export function buildReviewByExperimentMap(reviewSummary: ReviewSummary): ReviewByExperiment {
  ...
  for (const c of reviewSummary.counts) {
    map.set(c.experimentId, { complete: c.complete, needsReview: c.needsReview, total: c.total });
  }
  return map;
}
...
export function buildReviewByDatasetMap(
  ...
  if (!byExperiment.size || !experiments) {
    return map;
  }
  for (const exp of experiments) {
    const review = byExperiment.get(exp.id);
    if (!review || !exp.datasetId) {
      continue;
    }
    ...
    const existing = map.get(exp.datasetId) ?? { complete: 0, needsReview: 0 };

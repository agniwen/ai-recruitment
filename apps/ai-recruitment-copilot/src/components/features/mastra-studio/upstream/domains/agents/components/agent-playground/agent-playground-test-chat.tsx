...
import { DatasetSaveProvider } from "@/components/features/mastra-studio/upstream/lib/ai-ui/context/dataset-save-context";
...
function UnsavedChangesBanner({
  ...
  ctx: NonNullable<ReturnType<typeof useOptionalAgentEditFormContext>>;
}) {
  const { isDirty } = useFormState({ control: ctx.form.control });
  const { handleSaveDraft } = ctx;

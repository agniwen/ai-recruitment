...
import { ToolIconMap } from "@/components/features/mastra-studio/upstream/domains/tools/components/tool-icon";
...
export const MCPDetail = ({ isLoading, server }: MCPDetailProps) => {
  ...
  useEffect(() => {
    ...
    const nextSseUrl = getEmbeddedMastraApiUrl(`/mcp/${server.id}/sse`);
    const nextHttpStreamUrl = getEmbeddedMastraApiUrl(`/mcp/${server.id}/mcp`);

    setUrls({ httpStreamUrl: nextHttpStreamUrl, sseUrl: nextSseUrl });
  }, [server]);

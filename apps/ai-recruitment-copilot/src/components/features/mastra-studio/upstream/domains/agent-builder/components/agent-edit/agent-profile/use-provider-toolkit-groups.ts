 ...
 */
export const useProviderToolkitGroups = (
  availableAgentTools: AgentTool[],
): ProviderToolkitGroups => {
  ...
  return useMemo(() => {
    ...
    const providerSections: ProviderSection[] = (providers ?? [])
      ...
      .map((p) => ({
        presentSlugs: [...(slugsByProvider.get(p.id) ?? new Set<string>())],
        providerId: p.id,
        providerName: p.name,
      }));
    ...
    const allToolkitIds = [
      ...builtIn.map((t) => t.id),
      ...providerSections.flatMap((s) => s.presentSlugs),
    ];

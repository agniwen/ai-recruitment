...
export function DatasetExperimentsComparison({
  ...
  const {
    ...
    isLoading,
    error,
  } = useCompareExperiments(datasetId, experimentIdA, experimentIdB);
  ...
  const scorerSummaries = useMemo(() => {
    if (!comparison || scorerIds.length === 0) {
      return [];
    }
  ...
  const toNextItem = (): (() => void) | undefined => {
    if (!comparison || !featuredItemId) {
      return undefined;
    }
    const currentIndex = comparison.items.findIndex((i) => i.itemId === featuredItemId);
    if (currentIndex !== -1 && currentIndex < comparison.items.length - 1) {

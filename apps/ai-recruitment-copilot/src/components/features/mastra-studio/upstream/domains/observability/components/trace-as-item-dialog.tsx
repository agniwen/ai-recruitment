...
interface TraceAsItemDialogProps {
...
function getInitialInput(traceDetails?: SpanRecord): string {
  ...
  const rawInput = isWrappedAgentInput
    ? (spanInput.messages ?? traceDetails.input)
    : traceDetails.input;
...
export function TraceAsItemDialog({
  ...
  return (
    <SaveAsDatasetItemDialog
      ...
      onClose={onClose}
      level={level}
      source={traceId ? { referenceId: traceId, type: "trace" } : undefined}
    />
  );
}

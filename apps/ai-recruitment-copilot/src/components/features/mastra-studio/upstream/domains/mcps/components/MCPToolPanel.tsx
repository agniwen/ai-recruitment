...
import {
  useExecuteMCPTool,
  useMCPServerTool,
} from "@/components/features/mastra-studio/upstream/domains/mcps/hooks/use-mcp-server-tool";
import ToolExecutor from "@/components/features/mastra-studio/upstream/domains/tools/components/ToolExecutor";
...
export const MCPToolPanel = ({ toolId, serverId }: MCPToolPanelProps) => {
  ...
  let zodInputSchema;
  try {
    zodInputSchema = resolveSerializedZodOutput(
      jsonSchemaToZod(tool.inputSchema as unknown as JsonSchema),
    );
  } catch (error) {
    console.error("Error processing input schema:", error);

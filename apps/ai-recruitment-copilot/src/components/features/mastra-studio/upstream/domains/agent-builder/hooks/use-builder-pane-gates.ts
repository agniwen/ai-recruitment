...
import { useChannelPlatforms } from "@/components/features/mastra-studio/upstream/domains/agents/hooks/use-channels";
 ...
 */
export const useBuilderPaneGates = ({
  hasAgentTools,
  hasSkills,
}: BuilderPaneGatesInput): BuilderPaneGates => {
  ...
  const { browser } = features;
  const integrations = channelPlatforms.some(
    (platform) => platform.id === "slack" && platform.isConfigured,
  );

  return useMemo(
    () => ({ browser, integrations, model, skills, tools }),

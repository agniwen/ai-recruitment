...
interface FavoriteContext {
  ...
  previousLists: [unknown, ListStoredSkillsResponse | undefined][];
}

const applyFavoriteToSkill = (
  ...
  return { ...skill, favoriteCount: nextCount, isFavorited: favorited };
};
...
export const useToggleStoredSkillFavorite = (skillId?: string) => {
  ...
  return useMutation<FavoriteToggleResponse, Error, { favorited: boolean }, FavoriteContext>({
    ...
    onMutate: async ({ favorited }) => {
      ...
      const previousDetail = queryClient.getQueryData<StoredSkillResponse | null>([
        "stored-skill",
        skillId,
      ]);

      const listQueries = queryClient.getQueriesData<ListStoredSkillsResponse>({
      ...
      for (const [key, value] of listQueries) {
        ...
        queryClient.setQueryData<ListStoredSkillsResponse>(key as readonly unknown[], {
          ...
          skills: value.skills.map((s) =>
            s.id === skillId ? applyFavoriteToSkill(s, favorited) : s,
          ),
        });
      }

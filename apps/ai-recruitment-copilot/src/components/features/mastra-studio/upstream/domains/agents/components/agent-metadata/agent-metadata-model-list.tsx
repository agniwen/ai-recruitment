    ...
    <div className="rounded-lg bg-surface1 hover:bg-surface4/50 transition-colors">
      <div className="flex items-center gap-2 p-2">
        {showDragHandle && (
          <div
            {...dragHandleProps}
            className="text-neutral3 cursor-grab active:cursor-grabbing shrink-0"
          >
            <Icon>
              <GripVertical />
            </Icon>
        ...
        <div className="flex-1 min-w-0">
          <AgentMetadataModelSwitcher
            ...
            updateModel={(params) =>
              updateModelInModelList({ model: params, modelConfigId: modelConfig.id })
            }
            autoSave={true}
          />
        </div>
        {isLastEnabled ? (
          ...
          <Switch
            ...
            onCheckedChange={(checked) => {
              setEnabled(checked);
              void updateModelInModelList({ enabled: checked, modelConfigId: modelConfig.id });
            }}
            className="shrink-0"
          />
        )}

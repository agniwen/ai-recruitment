...
import { useLocation, useNavigate } from "@/components/features/mastra-studio/router/compat";
...
export const useStarterUserMessage = (): string | undefined => {
  ...
  const [userMessage] = useState<string | undefined>(
    () => (location.state as StarterLocationState)?.userMessage,
  );

...
interface ExperimentTraceTimelineToolsProps {
...
export function ExperimentTraceTimelineTools({
  ...
  const usedSpanTypes =
    spanTypePrefixes.filter((typePrefix) =>
      spans.some((span) => span?.spanType?.startsWith(typePrefix)),
    ) || [];

  const hasOtherSpanTypes = spans.some((span) => {
    const isKnownType = spanTypePrefixes.some((typePrefix) =>
      span?.spanType?.startsWith(typePrefix),
    );
    return !isKnownType;
  });

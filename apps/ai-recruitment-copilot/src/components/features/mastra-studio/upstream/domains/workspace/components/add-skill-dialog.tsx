...
interface SkillListContentProps {
  displaySkills: SkillsShSkill[];
  installedSkillIds: string[];
  installedSkillNames: string[];
  isLoading: boolean;
  onSelect: (skill: SkillsShSkill) => void;
  selectedSkill: SkillsShSkill | null;
}
...
interface SkillPreviewContentProps {
  isLoading: boolean;
  parsedSource: ReturnType<typeof parseSkillSource>;
...
function InstallActions({
  ...
  const hasMultipleMounts = Boolean(writableMounts && writableMounts.length > 1);
  const skillPath = installedSkillPaths?.[selectedSkill.name];
  const installedMount = skillPath
    ? writableMounts?.find((mount) =>
        Boolean(skillPath.startsWith(`${mount.path}/`) || skillPath === mount.path),
      )
    : undefined;
  return (
    <div className="flex flex-col gap-3 pt-4 border-t border-border1">
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <Button
          variant="primary"
          onClick={onInstall}
          disabled={!parsedSource || isInstalling || isInstalled}
          data-testid="install-skill-button"
        >
          <InstallButtonLabel installed={isInstalled} installing={isInstalling} />
        </Button>
      </div>
    </div>
  );
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-4xl h-[80vh] flex flex-col">
        ...
        <DialogBody className="flex-1 flex flex-col gap-4 overflow-hidden max-h-none">
            ...
            <div className="w-1/2 flex flex-col min-h-0">
              ...
              <ScrollArea className="flex-1 border border-border1 rounded-lg">
                <SkillListContent
                  ...
                  isLoading={isLoadingPopular || isSearching}
          ...
          <InstallActions
            installedSkillPaths={installedSkillPaths}
            isInstalled={isSelectedSkillInstalled}
            isInstalling={Boolean(isInstalling)}
            onCancel={() => handleOpenChange(false)}
            onInstall={handleInstall}
            onMountChange={setSelectedMount}
            parsedSource={parsedSource}

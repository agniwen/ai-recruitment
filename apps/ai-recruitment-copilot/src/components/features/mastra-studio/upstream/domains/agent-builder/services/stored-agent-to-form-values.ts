...
import { extractFormToolProviders } from "@/components/features/mastra-studio/upstream/domains/tool-providers/mappers/tool-providers-form-mappers";
 ...
 */
export function extractStaticModel(
  model: StoredAgent["model"] | undefined,
): AgentBuilderModel | undefined {
  if (!model || Array.isArray(model)) {
    return undefined;
  }
  const { provider } = model as { provider?: unknown };
  const { name } = model as { name?: unknown };
  if (
...
export function storedAgentToFormValues(
  ...
  return {
    ...
    skills: Object.fromEntries(
      Object.keys(flattenAgentSkills(storedAgent?.skills)).map((k) => [k, true]),
    ),

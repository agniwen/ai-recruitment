...
export const MCPToolPanel = ({ toolId, serverId }: MCPToolPanelProps) => {
  ...
  const handleExecuteTool = async (data: unknown) => {
    if (!tool) {
      return;
    }
  ...
  try {
    zodInputSchema = resolveSerializedZodOutput(
      tool.inputSchema as Parameters<typeof resolveSerializedZodOutput>[0],
    );
  } catch (schemaError) {
    console.error("Error processing input schema:", schemaError);
    toast.error("处理工具输入 Schema 失败。");

...
import { GenerateItemsDialog } from "@/components/features/mastra-studio/upstream/domains/datasets/components/generate-items-dialog";
...
function ExperimentBadge({ status, passRate }: { status: string; passRate: number | null }) {
  ...
  const color =
    passRate >= 80 ? "text-green-500" : passRate >= 50 ? "text-yellow-400" : "text-red-400";

...
function upsertSegment(
  ...
  const next = existing
    ? segments.map((segment) =>
        segment.id === id
          ? { ...segment, final: segment.final || final, text: segment.text + chunk }
          : segment,
      )
    : [...segments, { final, id, role, text: chunk }];
...
export const useVoiceCall = ({
  ...
  threadId,
  onCallStarted,
}: UseVoiceCallArgs): VoiceCallControls => {
  ...
  const refreshThread = useCallback(() => {
    if (threadId) {
      void queryClient.invalidateQueries({ queryKey: ["memory", "messages", threadId] });
    }
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      room.on(RoomEvent.ParticipantAttributesChanged, (changed, participant) => {
        const state =
          changed[AGENT_STATE_ATTRIBUTE] ?? participant.attributes[AGENT_STATE_ATTRIBUTE];

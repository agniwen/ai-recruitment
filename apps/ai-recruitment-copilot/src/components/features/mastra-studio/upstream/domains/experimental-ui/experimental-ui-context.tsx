...
export interface UIExperimentVariantOption {
  value: string;
  label: string;
}

export interface UIExperimentConfig {
  key: string;
  name: string;
  path?: string | string[];
...
interface ExperimentalUIContextValue {
...
export function ExperimentalUIProvider({
  ...
  return (
    <ExperimentalUIContext.Provider value={contextValue}>{children}</ExperimentalUIContext.Provider>
  );
}

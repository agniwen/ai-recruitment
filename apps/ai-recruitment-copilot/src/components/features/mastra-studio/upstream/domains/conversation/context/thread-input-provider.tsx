...
export const ThreadInputProvider = ({ children }: { children: ReactNode }) => {
  ...
  const setThreadInputForThread = useCallback(
    (threadId: string | undefined, value: SetStateAction<string>) => {
      setThreadInputs((prev) => {
        ...
        if (nextValue.length === 0) {
          ...
          return Object.fromEntries(Object.entries(prev).filter(([inputKey]) => inputKey !== key));
        }
  ...
  const value = useMemo(
    () => ({ getThreadInput, setThreadInputForThread }),
    [getThreadInput, setThreadInputForThread],
  );

...
function parseField(key: string, schema: AnySchema): ParsedField {
  ...
  if (intersection) {
    ...
    if (leftShape) {
      subSchemaLeft = Object.entries(leftShape).map(([k, field]) =>
        parseField(k, field as AnySchema),
      );
    } else {
    ...
    if (rightShape) {
      subSchemaRight = Object.entries(rightShape).map(([k, field]) =>
        parseField(k, field as AnySchema),
      );
    } else {
...
export function parseSchema(schema: AnySchema): ParsedSchema {
  ...
  const fields: ParsedField[] = Object.entries(shape).map(([key, field]) =>
    parseField(key, field as AnySchema),
  );

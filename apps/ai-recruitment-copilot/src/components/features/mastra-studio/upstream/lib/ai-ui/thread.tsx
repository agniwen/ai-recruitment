...
} from "./attachments/composer-attachments";
...
const hasStreamingPart = (message: MastraDBMessage | undefined) => {
  ...
  const parts = message.content.parts as MessageFactoryPart[];
...
function Composer({
  ...
  const voiceCall = useVoiceCall({ agentId, onCallStarted: refreshThreadList, threadId });

  const isEmpty = text.trim().length === 0 && attachments.length === 0;
  const sendBlocked = isRunning && !canSendWhileStreaming;

  const submit = async () => {
    if (isEmpty || sendBlocked || !canExecuteAgent) {
      return;
    }
    ...
    send({ attachments: coreUserMessages, message });
  };
      ...
      >
        ...
        >
          ...
          <div className="relative z-10">
            ...
            {agentId && !hasModelList && !hideModelSwitcher && (
              <ComposerModelWarning agentId={agentId} />
            )}
            <ComposerActionRow

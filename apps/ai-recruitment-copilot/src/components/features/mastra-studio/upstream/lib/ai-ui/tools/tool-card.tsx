...
import { useMcpAppTools } from "@/components/features/mastra-studio/upstream/domains/mcps/hooks";
...
export const ToolCardInner = ({
  ...
  const agentToolName = toolName.startsWith("agent-")
    ? toolName.substring("agent-".length)
    : toolName;
  const workflowToolName = toolName.startsWith("workflow-")
    ? toolName.substring("workflow-".length)
    : toolName;

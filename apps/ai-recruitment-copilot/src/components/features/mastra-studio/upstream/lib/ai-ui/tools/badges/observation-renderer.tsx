...
function parseObservations(raw: string): ParsedObservations {
  ...
  for (const section of threadSections) {
    ...
    const content = threadMatch
      ? trimmedSection.replace(/^Thread [a-f0-9]+:\s*/i, "")
      : trimmedSection;
...
function ObservationItem({
  ...
  const borderColor = observation.priority
    ? PRIORITY_BORDER[observation.priority]
    ...
    <div className="mb-2">
      ...
      >
        <span
          className={cn(
            "text-xs font-medium",
            useInheritedTextColor ? "opacity-80" : "text-foreground",
          )}
        >
          {block.date}
        </span>

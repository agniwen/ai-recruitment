...
const PERIOD_MS: Record<EventCalendarRecurrenceRule["freq"], number> = {
  daily: 86_400_000,
  monthly: 2_629_746_000, // 365.2425 / 12 days
  weekly: 604_800_000,
  yearly: 31_556_952_000, // 365.2425 days
};
...
function parseRRuleString(input: string, timeZone?: string): EventCalendarRecurrenceRule {
    ...
    switch (key) {
      ...
      case "BYDAY": {
        rule.byWeekday = value.split(",").map((token) => {
          ...
          if (!match) {
            throw new EventCalendarRecurrenceError(`BYDAY=${token}`);
          }
          const day = match[2] as EventCalendarWeekday;
          return match[1] ? { day, ordinal: Number.parseInt(match[1], 10) } : day;
        });
        break;
      }
      case "BYMONTHDAY": {
  ...
  if (!rule.freq) {
    throw new EventCalendarRecurrenceError("missing FREQ");
  }
...
function expandRecurrence<TData>(
  ...
  const yearlyMonths: number[] =
    validByMonth.length > 0
      ? [...new Set(validByMonth)].toSorted((a, b) => a - b)
  ...
  const candidatesFor = (period: number): TZDate[] => {
    if (rule.freq === "daily") {
      return [addDays(zonedStart, period * interval)];
    }
    if (rule.freq === "weekly") {
      ...
      for (let d = 0; d < 7; d++) {
        ...
        if (candidate.getTime() < zonedStart.getTime()) {
          continue;
        }
        week.push(candidate);
      }
  ...
  const periodEdge = (period: number, edge: "first" | "last"): TZDate => {
    ...
    const year = addYears(zonedStart, period * interval).getFullYear();
    const month = ((edge === "first" ? yearlyMonths[0] : yearlyMonths.at(-1)) ?? 1) - 1;
    return zonedDate(year, month, edge === "first" ? 1 : daysInMonth(year, month));
  };
  ...
  while (iterations < MAX_ITERATIONS && occurrences.length < MAX_OCCURRENCES) {
    ...
    if (earliest.getTime() >= range.end.getTime()) {
      break;
    }
    if (rule.until && earliest.getTime() > rule.until.getTime()) {
      break;
    }
  ...
  if (rule.rDates?.length) {
    ...
    for (const rDate of rule.rDates) {
      ...
      if (seen.has(start.getTime()) || exTimes.has(start.getTime())) {
        continue;
      }
      const end = new Date(start.getTime() + durationMs);
      if (start >= range.end || end <= range.start) {

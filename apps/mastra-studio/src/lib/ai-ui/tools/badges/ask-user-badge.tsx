...
export const AskUserBadge = ({ toolCallId, suspendPayload, result }: AskUserBadgeProps) => {
  ...
  const handleMultiSubmit = useCallback(() => {
    if (selectedOptions.length === 0 || isAnswered || isRunning) {
      return;
    }
  ...
  const handleFreeTextSubmit = useCallback(() => {
    ...
    if (!trimmed || isAnswered || isRunning) {
      return;
    }
            ...
            <div className="flex flex-col gap-2">
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected
                        ? "border-accent1 bg-surface4"
                        : "border-border1 bg-surface4 hover:bg-surface5",
                    )}
                  >
                    <Txt as="span" variant="ui-md" className="font-medium text-neutral6">
                      {option.label}
              ...
              {resolvedMode === "multi_select" && (
                <Button
                  ...
                  disabled={selectedOptions.length === 0 || isRunning}
                >
                  <Icon>
                    <Send />
                  </Icon>

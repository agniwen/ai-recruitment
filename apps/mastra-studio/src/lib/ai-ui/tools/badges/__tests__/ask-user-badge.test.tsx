...
interface ProviderOverrides {
  ...
  isRunning?: boolean;
  toolCallApprovals?: Record<string, { status: "approved" | "declined" }>;
}
...
describe("AskUserBadge", () => {
  describe("when the payload offers single-select options", () => {
    const suspendPayload: AskUserSuspendPayload = {
      options: [{ description: "A red fruit", label: "Apple" }, { label: "Banana" }],
      question: "Pick a fruit",
      selectionMode: "single_select",
    };

    it("renders the question and the options", () => {
      renderBadge({ result: undefined, suspendPayload, toolCallId: "call-1" });
  ...
  describe("when the payload offers multi-select options", () => {
    ...
    it("keeps the submit button disabled until something is selected", () => {
      renderBadge({ result: undefined, suspendPayload, toolCallId: "call-2" });

      expect(
        (within(badge()).getByRole("button", { name: /submit/i }) as HTMLButtonElement).disabled,
      ).toBe(true);
    });
  ...
  describe("when the tool call has already been answered", () => {
    ...
    it("hides the inputs when the approval status is approved", () => {
      renderBadge(
        { result: undefined, suspendPayload, toolCallId: "call-4" },
  ...
  describe("when a tool call is already running", () => {
    it("disables the option buttons and does not submit on click", () => {
      ...
      const { approveToolcall } = renderBadge(
        { result: undefined, suspendPayload, toolCallId: "call-5" },
        { isRunning: true },
      );

      const optionButton = within(badge()).getByRole("button", {
        name: "Apple",
    ...
    it("disables the free-text input", () => {
      renderBadge(
        { result: undefined, suspendPayload: { question: "Name?" }, toolCallId: "call-6" },
        { isRunning: true },
      );

      expect(
        (within(badge()).getByPlaceholderText("Type your answer...") as HTMLInputElement).disabled,
      ).toBe(true);
    });
  });
  ...
  describe("when the question is answered", () => {
    it("shows the Answered status pill", () => {
      renderBadge({
        result: { content: "User answered: Apple", isError: false },
        suspendPayload: { options: [{ label: "Apple" }], question: "Pick a fruit" },
        toolCallId: "call-7",
      });

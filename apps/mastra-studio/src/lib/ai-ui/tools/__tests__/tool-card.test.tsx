...
describe("ToolCard dispatch", () => {
  ...
  it("uses streaming OM output data for completed observation markers even when metadata has stale start data", () => {
    renderToolCard(
      baseProps({
        input: { _state: "loading", cycleId: "cycle-stream", operationType: "observation" },
        metadata: {
          mode: "stream",
          omData: { _state: "loading", cycleId: "cycle-stream", operationType: "observation" },
        },
        output: {
          omData: {
            _state: "complete",
  ...
  it("routes workflow-* tools to the workflow badge", async () => {
    server.use(
      http.get(`${BASE_URL}/api/workflows/myFlow`, () =>
        HttpResponse.json({ allSteps: {}, name: "My Flow", stepGraph: [], steps: {} }),
      ),
    );
    renderToolCard(
      baseProps({
  ...
  it("pushes a streaming workflow output into WorkflowRunContext for the live graph", async () => {
    server.use(
      http.get(`${BASE_URL}/api/workflows/liveFlow`, () =>
        HttpResponse.json({ allSteps: {}, name: "Live Flow", stepGraph: [], steps: {} }),
      ),
    );

...
export function RouteHeaderActionsProvider({ children }: { children: ReactNode }) {
  ...
  const register = useCallback((owner: string, priority: number) => {
    ...
    return () => {
      setOwners((current) =>
        current.filter((entry) => !(entry.owner === owner && entry.order === order)),
      );
    };
  }, []);
  ...
  const value = useMemo(() => ({ activeOwner, el, register, setEl }), [activeOwner, el, register]);
...
export function RouteHeaderActions({
  ...
  if (!ctx?.el || ctx.activeOwner !== owner) {
    return null;
  }

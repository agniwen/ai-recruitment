...
function getAppRoutes() {
  const rootRoute = routes.find((route) =>
    route.children?.some((child) => child.path === "/agents"),
  );
...
function joinPath(basePath: string, route: RouteObject) {
  ...
  return `${normalizedBase}/${route.path}`.replaceAll(/\/+/g, "/");
}
...
function collectRoutesMissingCrumbs(
  routesToCheck: RouteObject[],
  basePath = "",
  hasParentCrumbs = false,
) {
...
function resolveCrumbs(routePath: string, handle: RouteHeaderHandle): CrumbDef[] {
  ...
  if (typeof handle.crumbs === "function") {
    return handle.crumbs({
      ...
      pathname: routePath.replaceAll(/:([A-Za-z0-9_]+)/g, "$1-fixture"),
    });
  }
...
function collectRouteHandles(routesToCheck: RouteObject[], basePath = "") {
  const handles: { path: string; handle: RouteHeaderHandle }[] = [];
  ...
  for (const route of routesToCheck) {
    ...
    if (handle) {
      handles.push({ handle, path: routePath });
    }
...
describe("route header handles", () => {
  ...
  it("resolves declared breadcrumb handles to accessible page headings", () => {
    const invalidHandles = collectRouteHandles(getAppRoutes()).flatMap(({ path, handle }) => {
      const crumbs = resolveCrumbs(path, handle);
      const hasUntitledComponentCrumb = crumbs.some(
        (crumb) => "Component" in crumb && !crumb.heading?.trim(),
      );
  ...
  it("does not throw when route params contain malformed URI encoding", () => {
    ...
    expect(() => {
      if (typeof scheduleHandle?.crumbs !== "function") {
        return;
      }
          ...
          children: [
            {
              ...
              handle: { crumbs: [{ id: "child", label: "Child" }], docs: () => {} },
              path: "child",
            },
          ],
          element: <Outlet />,

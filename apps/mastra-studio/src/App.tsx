...
import {
  AgentBuilderEditionLayout,
  AgentBuilderLayout,
} from "@/domains/agent-builder/layouts/agent-builder-layout";
...
const paths: LinkComponentProviderProps["paths"] = {
  ...
  networkThreadLink: (networkId: string, threadId: string) =>
    `/networks/v-next/${networkId}/chat/${threadId}`,
  ...
  workspaceLink: (workspaceId?: string) =>
...
const agentCmsChildRoutes = [
  { element: <CmsAgentInformationPage />, index: true },
  { element: <CmsAgentInstructionBlocksPage />, path: "instruction-blocks" },
  { element: <CmsAgentToolsPage />, path: "tools" },
  { element: <CmsAgentAgentsPage />, path: "agents" },
  { element: <CmsAgentScorersPage />, path: "scorers" },
  { element: <CmsAgentWorkflowsPage />, path: "workflows" },
  { element: <CmsAgentSkillsPage />, path: "skills" },
  { element: <CmsAgentMemoryPage />, path: "memory" },
  { element: <CmsAgentVariablesPage />, path: "variables" },
];
...
export const routes: RouteObject[] = [
  ...
  { element: <SignUp />, path: "/signup" },
  {
    children: [
      ...
      {
        children: [
          { element: <AgentBuilderCreate />, path: "create" },
          {
            loader: ({ params }: LoaderFunctionArgs) =>
              redirect(`/agent-builder/agents/${params.id}/view`),
            path: ":id",
          },
          { element: <AgentBuilderAgentEdit />, path: ":id/edit" },
          { element: <AgentBuilderAgentView />, path: ":id/view" },
        ],
      ...
      {
        children: [
          { element: <AgentBuilderSkillsCreate />, path: "create" },
          {
            loader: ({ params }: LoaderFunctionArgs) =>
              redirect(`/agent-builder/skills/${params.id}/edit`),
            path: ":id",
          },
          { element: <AgentBuilderSkillsEdit />, path: ":id/edit" },
          { element: <AgentBuilderSkillsView />, path: ":id/view" },
        ],
        element: <AgentBuilderEditionLayout />,
        path: "skills",
      },
  ...
  {
    children: [
      { element: <AgentSession />, path: "/agents/:agentId/session" },
      { element: <AgentSession />, path: "/agents/:agentId/session/:threadId" },
    ],
  ...
  },
  {
    children: [
      ...
      ...(isMastraPlatform
        ? []
        : [
            { element: <StudioSettingsPage />, handle: navHandle("/settings"), path: "/settings" },
            {
              element: <Templates />,
      ...
      { element: <Logs />, handle: navHandle("/logs"), path: "/logs" },
      { element: <Evaluation />, handle: navHandle("/evaluation"), path: "/evaluation" },
      { element: <Scorers />, handle: navHandle("/scorers"), path: "/scorers" },
      {
        element: <Scorer />,
      ...
      { element: <Metrics />, handle: navHandle("/metrics"), path: "/metrics" },
      { element: <Traces />, handle: navHandle("/observability"), path: "/observability" },
      {
      ...
      { element: <Resources />, handle: navHandle("/resources"), path: "/resources" },
      { element: <Agents />, handle: navHandle("/agents"), path: "/agents" },
      {
      ...
      },
      {
        ...
        handle: navHandleWithChildren("/scorers", [
          { id: "create-scorer", label: "Create scorer" },
        ]),
      ...
      { element: <PromptBlocks />, handle: navHandle("/prompts"), path: "/prompts" },
      {
      ...
      },
      {
        children: [
          ...
          { element: <Agent />, path: "chat" },
          { element: <Agent />, path: "chat/:threadId" },
          { element: <Agent view="settings" />, path: "settings" },
          ...(isExperimentalFeatures
            ? [
                { element: <AgentPlayground />, path: "editor" },
                { element: <AgentEvaluate />, path: "evaluate" },
                { element: <AgentReview />, path: "review" },
              ]
            : []),
          { element: <AgentTraces />, path: "traces" },
          {
            // Channels is configuration, not a tool tab: it now lives in the
            // agent settings view. Keep old links working.
            path: "channels",
            loader: ({ params }: LoaderFunctionArgs) =>
              redirect(`/agents/${params.agentId}/settings?tab=channels`),
          },
        ],
        element: (
          <AgentLayout>
      ...
      { element: <Tools />, handle: navHandle("/tools"), path: "/tools" },
      {
        element: <Tool />,
      ...
      { element: <Processors />, handle: navHandle("/processors"), path: "/processors" },
      {
        element: <Processor />,
      ...
      { element: <MCPs />, handle: navHandle("/mcps"), path: "/mcps" },
      {
      ...
      { element: <Workspace />, handle: navHandle("/workspaces"), path: "/workspaces" },
      {
        element: <Workspace />,
          ...
          crumbs: ({ params }) => [
            navCrumb("/workspaces"),
            {
              ...
              to: params.workspaceId
                ? `/workspaces/${encodeURIComponent(params.workspaceId)}`
                : undefined,
            },
      ...
      { element: <Workflows />, handle: navHandle("/workflows"), path: "/workflows" },
      {
      ...
      },
      {
        ...
        handle: {
          crumbs: ({ params }) => [
            navCrumb("/workflows"),
            schedulesCrumb,
            { icon: CalendarClockIcon, id: "schedule", label: decodeRouteParam(params.scheduleId) },
          ],
        } satisfies RouteHeaderHandle,
      ...
      },
      {
        children: [
          ...
          { element: <Workflow />, path: "graph" },
          {
            element: <Workflow />,
            handle: {
              crumbs: [
                { Component: WorkflowRunCrumb, heading: "Workflow run", id: "workflow-run" },
              ],
            } satisfies RouteHeaderHandle,
            path: "graph/:runId",
          },
      ...
      ...(isExperimentalFeatures
        ? [
            { element: <Datasets />, handle: navHandle("/datasets"), path: "/datasets" },
            {
              element: <DatasetPage />,
              handle: {
                crumbs: () => [
                  navCrumb("/datasets"),
                  { Component: DatasetCrumb, heading: "Dataset", id: "dataset" },
                ],
              } satisfies RouteHeaderHandle,
            ...
            { element: <Experiments />, handle: navHandle("/experiments"), path: "/experiments" },
            {
      ...
      },
      {
        ...
        handle: navHandle("/request-context"),
...
export default function AppWrapper() {
  ...
  const endpoint =
    cloudApiEndpoint || (autoDetectUrl ? window.location.origin : `${protocol}://${host}:${port}`);

...
describe("AddTraceMocksToItemDialog", () => {
  ...
  it("appends derived mocks to the existing item on submit (existing ++ derived)", async () => {
    ...
    server.use(
      http.get(`${BASE_URL}/api/datasets`, () => HttpResponse.json(datasetsList)),
      http.get(`${BASE_URL}/api/datasets/dataset-1/items`, () =>
        HttpResponse.json(datasetItemsList),
      ),
      http.get(`${BASE_URL}/api/datasets/dataset-1/items/item-1`, () =>
        HttpResponse.json(datasetItem),
      ),
      http.get(`${BASE_URL}/api/observability/traces/${TRACE_ID}/trajectory`, () =>
        HttpResponse.json(trajectoryWithToolCalls),
      ),
    ...
    await waitFor(() => {
      ...
      expect([...datasetSelect.options].some((o) => o.value === "dataset-1")).toBe(true);
    });
    ...
    await waitFor(
      () => {
        ...
        expect([...itemSelect.options].some((o) => o.value === "item-1")).toBe(true);
      },
      { timeout: 3000 },
    );
    ...
    expect(capture.mock.calls[0][0]).toMatchObject({
      toolMocks: [
        { args: { a: 1 }, output: { ok: true }, toolName: "existing" },
        { args: { city: "Seattle" }, output: { temp: 52 }, toolName: "getWeather" },
      ],
    });
  });
  ...
  it("disables submit when the trace has no tool calls (empty editor)", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /append tool mocks/i }).hasAttribute("disabled"),
    ).toBe(true);
  });
    ...
    fireEvent.change(editor, {
      target: {
        value: JSON.stringify([
          { args: { city: "Paris" }, output: { temp: 70 }, toolName: "getWeather" },
        ]),
      },
    });

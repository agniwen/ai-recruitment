...
const renderAutosave = ({
  ...
  const view = renderHook(
    () => useAutosaveAgent({ agentId: AGENT_ID, debounceMs, savedDisplayMs }),
    {
      wrapper: Wrapper,
    },
  );
...
describe("useAutosaveAgent", () => {
  beforeEach(() => {
    ...
    server.use(
      http.get(`${BASE_URL}/api/auth/capabilities`, () =>
        HttpResponse.json(authEnabledCapabilities),
      ),
    );
  });
  ...
  describe("when the server fails the save", () => {
    it("reports an error and recovers via retry", async () => {
      ...
      await waitFor(() => expect(result.current.status).toBe("saved"), { timeout: 5000 });
  ...
  describe("when flushNow is called with a pending edit", () => {
    it("fires the PATCH immediately instead of waiting for the debounce", async () => {
      ...
      const { result, form } = renderAutosave({ debounceMs: 5000 });

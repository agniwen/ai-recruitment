...
const seedWritableAuth = () => {
  server.use(
    http.get(`${BASE_URL}/api/auth/capabilities`, () =>
      HttpResponse.json(authEnabledWritableCapabilities),
    ),
  );
};
  ...
  server.use(
    http.post(`${BASE_URL}/api/stored/skills`, async ({ request }) => {
      ...
      return HttpResponse.json(
        makeStoredSkill({ description: body.description, id, name: body.name }),
      );
    }),
  );
  return calls;
...
const seedWorkspaceWrite = () => {
  ...
  server.use(
    http.post(`${BASE_URL}/api/workspaces/:workspaceId/fs/write`, ({ params }) => {
      ...
      return HttpResponse.json({ path: String(params.workspaceId), success: true });
    }),
  );
  return workspaceIds;
};
...
const renderCreateSkillTool = (
  options: { availableWorkspaces?: { id: string; name: string }[] } = {},
) => {
  ...
  return { form: () => formRef.current!, result };
};
...
describe("useCreateSkillTool", () => {
  describe("when a workspace is available", () => {
    it("creates a stored skill from the input with the default private visibility", async () => {
      ...
      expect(outcome).toEqual({ skillId: "skill-new", success: true });
    ...
    it("attaches the created skill to the form", async () => {
      ...
      const { result, form } = renderCreateSkillTool({
        availableWorkspaces: [{ id: "ws-1", name: "Primary" }],
      });
      await waitFor(() => expect(result.current.visibility).toBe("private"));

...
const renderAutosave = ({
  ...
  const formRef: { current: ReturnType<typeof useForm<SkillEditFormValues>> | null } = {
    current: null,
  };
  ...
  const view = renderHook(
    () => useAutosaveSkill({ debounceMs, savedDisplayMs, skillId: SKILL_ID }),
    {
      wrapper: Wrapper,
    },
  );
    ...
    it("regenerates the SKILL.md file tree and PATCHes the stored skill", async () => {
      ...
      server.use(
        http.patch(`${BASE_URL}/api/stored/skills/${SKILL_ID}`, async ({ request }) => {
          ...
          return HttpResponse.json({ id: SKILL_ID, name: "Skill", status: "active" });
        }),
      );

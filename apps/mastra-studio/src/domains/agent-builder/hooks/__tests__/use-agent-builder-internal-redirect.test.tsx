...
describe("useAgentBuilderInternalRedirect", () => {
  it("reports isLoading=true initially and then hasAgents=false when both lists are empty", async () => {
    ...
    expect(result.current).toEqual({ hasAgents: false, isLoading: true });

    await waitFor(() => expect(result.current.isLoading).toBe(false));
    expect(result.current).toEqual({ hasAgents: false, isLoading: false });
  });

  it("reports hasAgents=true when only drafts exist", async () => {
    stubStoredAgents((status) =>
      HttpResponse.json(status === "draft" ? oneDraftAgent : emptyStoredAgents),
    );
    ...
    await waitFor(() => expect(result.current.isLoading).toBe(false));
    expect(result.current).toEqual({ hasAgents: true, isLoading: false });
  });

  it("reports hasAgents=true when only published agents exist", async () => {
    stubStoredAgents((status) =>
      HttpResponse.json(status === "published" ? onePublishedAgent : emptyStoredAgents),
    );
  ...
  it("reports hasAgents=true when both lists contain agents", async () => {
    stubStoredAgents((status) =>
      HttpResponse.json(status === "draft" ? oneDraftAgent : twoPublishedAgents),
    );
  ...
  it("treats responses missing the agents field as empty", async () => {
    ...
    server.use(
      http.get(`${BASE_URL}/api/stored/agents`, () =>
        HttpResponse.json({ hasMore: false, page: 1, perPage: 50, total: 0 }),
      ),
    );

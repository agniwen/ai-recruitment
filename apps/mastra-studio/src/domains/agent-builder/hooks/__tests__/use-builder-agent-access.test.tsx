...
const renderAccess = ({
  ...
  server.use(
    http.get(`${BASE_URL}/api/editor/builder/settings`, async () => {
      if (settingsDelayMs) {
        await delay(settingsDelayMs);
      }
      if (settingsStatus !== 200) {
        return HttpResponse.json({ error: "boom" }, { status: settingsStatus });
      }
  ...
  describe("when RBAC is enabled and the user has read and write (member role)", () => {
    ...
    it("exposes the agent feature flags from settings", async () => {
      const { result } = renderAccess({
        ...
        settings: {
          enabled: true,
          features: { agent: { memory: true, skills: false, tools: true } },
        },
      });
      ...
      expect(result.current.agentFeatures).toEqual({ memory: true, skills: false, tools: true });
    });
  });

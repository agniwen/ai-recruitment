...
const availableSkills = [
  { description: "first", id: "skill-1", name: "Skill One" },
  { description: "second", id: "skill-2", name: "Skill Two" },
...
const renderTool = (skills: StoredSkillResponse[] = availableSkills) => {
  ...
  const Wrapper = ({ children }: { children: React.ReactNode }) => {
    const methods = useForm<AgentBuilderEditFormValues>({
      defaultValues: { description: "", instructions: "", name: "", skills: {} },
    });
    formRef.current = methods;
    return React.createElement(FormProvider, methods, children);
  };

  const { result } = renderHook(() => useSetAgentSkillsTool({ availableSkills: skills }), {
    wrapper: Wrapper,
  });

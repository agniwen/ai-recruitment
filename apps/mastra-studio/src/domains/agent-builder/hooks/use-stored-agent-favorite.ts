...
interface FavoriteContext {
  ...
  previousLists: [unknown, ListStoredAgentsResponse | undefined][];
}

const applyFavoriteToAgent = (
  ...
  return { ...agent, favoriteCount: nextCount, isFavorited: favorited };
};
...
export const useToggleStoredAgentFavorite = (agentId?: string) => {
  ...
  return useMutation<FavoriteToggleResponse, Error, { favorited: boolean }, FavoriteContext>({
    ...
    onMutate: async ({ favorited }) => {
      ...
      // Snapshot detail
      const previousDetail = queryClient.getQueryData<StoredAgentResponse | null>([
        "stored-agent",
        agentId,
      ]);

      // Optimistically patch every matching list query
      const listQueries = queryClient.getQueriesData<ListStoredAgentsResponse>({
        queryKey: ["stored-agents"],
      });
      ...
      for (const [key, value] of listQueries) {
        ...
        queryClient.setQueryData<ListStoredAgentsResponse>(key as readonly unknown[], {
          ...
          agents: value.agents.map((a) =>
            a.id === agentId ? applyFavoriteToAgent(a, favorited) : a,
          ),
        });
      }

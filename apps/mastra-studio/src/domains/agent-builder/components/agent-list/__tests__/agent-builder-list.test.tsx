...
function renderList({ agents, search, ...props }: AgentBuilderListProps) {
  ...
  server.use(
    ...
    http.get(`${BASE_URL}/api/editor/builder/settings`, () =>
      HttpResponse.json(settingsFavoritesOn),
    ),
  );
...
describe("AgentBuilderList", () => {
  ...
  it("shows empty state when no rows match", () => {
    renderList({ agents: fixtureAgents, rowTestId: "agent-row", search: "zzz" });
  ...
  it("renders the resolved author name when the server returned `author`", () => {
    const agents: StoredAgentResponse[] = [
      {
        author: { avatarUrl: "https://x/y.png", id: "user-1", name: "Alice" },
        authorId: "user-1",
        createdAt: now,
        description: "desc",
        id: "a1",
  ...
  it("falls back to email when the resolved author has no name", () => {
    const agents: StoredAgentResponse[] = [
      {
        author: { email: "alice@example.com", id: "user-1" },
        authorId: "user-1",
        createdAt: now,
        description: "desc",
        id: "a1",

...
describe("ConnectChannelMessage", () => {
  ...
  it("renders nothing when agentId is missing", () => {
    server.use(
      platformsHandler([{ id: "slack", isConfigured: true, name: "Slack" }]),
    ...
    expect(
      container.querySelector('[data-testid="agent-builder-chat-connect-channel-slack"]'),
    ).toBeNull();
  });
  ...
  it('shows a disabled "Not configured" button when the platform is not configured', async () => {
    server.use(
      platformsHandler([{ id: "slack", isConfigured: false, name: "Slack" }]),
      installationsHandler({}),
    );

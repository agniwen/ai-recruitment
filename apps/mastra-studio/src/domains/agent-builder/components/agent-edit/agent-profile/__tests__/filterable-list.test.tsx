...
const renderList = (overrides: Partial<Parameters<typeof FilterableList>[0]> = {}) => {
  ...
  return { ...utils, onClearAll, onSelectAll, onToggle };
};
...
describe("FilterableList", () => {
  ...
  it("renders a row and checkbox per item", () => {
    ...
    for (const item of items) {
      ...
      expect(getByTestId(`list-filter-checkbox-${item.id}`).getAttribute("aria-checked")).toBe(
        "true",
      );
    }
  });
  ...
  it("disables controls when disabled", () => {
    ...
    expect(Object.hasOwn(getByTestId("list-filter-checkbox-alpha").dataset, "disabled")).toBe(true);
  });
});

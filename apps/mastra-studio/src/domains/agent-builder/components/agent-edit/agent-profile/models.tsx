      ...
      >
        ...
        <div className="grid grid-cols-1 content-start gap-2 lg:gap-6 sm:grid-cols-2 2xl:grid-cols-3">
          {group.models.map((entry) => {
            ...
            const isSelected =
              cleanedProvider === selectedProvider && entry.model === selectedModel;
...
function groupModelsByProvider(
  ...
  const filtered = modelInfo.filter((m) => {
    ...
    if (selectedProviders !== null && !selectedProviders.has(providerId)) {
      return false;
    }
    ...
    if (m.provider.toLowerCase().includes(searchLower)) {
      return true;
    }
    if (m.model.toLowerCase().includes(searchLower)) {
      return true;
    }
    return false;
  });
  ...
  for (const entry of filtered) {
    ...
    if (!group) {
      group = { models: [], providerId, providerName: entry.providerName };
  ...
  return [...groupsById.values()].toSorted((a, b) => {

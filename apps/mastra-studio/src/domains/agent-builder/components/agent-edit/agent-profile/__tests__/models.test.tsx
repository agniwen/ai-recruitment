...
const registerModelHandlers = () => {
  server.use(
    http.get(`${BASE_URL}/api/editor/builder/settings`, () =>
      HttpResponse.json(buildBuilderSettings()),
    ),
    http.get(`${BASE_URL}/api/editor/builder/models/available`, () =>
      HttpResponse.json(buildAvailableModels()),
    ),
  );
};
...
describe("Models", () => {
  ...
  describe("when rendering the provider filter", () => {
    it("renders one filter row per provider, all checked by default", async () => {
      ...
      expect(
        getByTestId("models-provider-filter-checkbox-openai").getAttribute("aria-checked"),
      ).toBe("true");
      expect(
        getByTestId("models-provider-filter-checkbox-anthropic").getAttribute("aria-checked"),
      ).toBe("true");
    });
  });

 ...
 */
const dedupeLabels = (
  pins: ToolProviderConnectionFormValue[],
): ToolProviderConnectionFormValue[] => {
...
const pinsEqual = (
  a: ToolProviderConnectionFormValue[],
  b: ToolProviderConnectionFormValue[],
): boolean => {
...
const getToolkitConnectionPins = ({
  ...
  return dedupeLabels(
    desired.map((pin) => ({ ...pin, label: pin.label || deriveConnectionLabel(pin.connectionId) })),
  );
};
...
const useSyncActiveConnectionsToForm = ({
  ...
  useEffect(() => {
    if (!hasSelectedTool || pinsEqual(currentPins, desiredPins)) {
      return;
    }
...
export const ToolkitConnectionControl = ({
  ...
  const pinnedRaw = useWatch({ name: connectionsField }) as
    | ToolProviderConnectionFormValue[]
    | undefined;
  ...
  useSyncActiveConnectionsToForm({
    connectionsField,
    currentPins: pinned,
    desiredPins,
    hasSelectedTool,
  ...
  const unpinConnection = (connectionId: string) => {
    if (!pinned.some((connection) => connection.connectionId === connectionId)) {
      return;
    }
    setValue(
      connectionsField,
  ...
  const handleConnect = () => {
    authorize.mutate(
      { providerId, scope: "per-author", toolkit },
      {
        onSuccess: () => {
  ...
  if (connectionsQuery.isPending) {
    return (
      <Skeleton className="h-7 w-20" data-testid={`toolkit-connection-loading-${testIdSuffix}`} />
    );
  }

...
const availableTools: AgentTool[] = [
  { id: "checked-tool", isChecked: true, name: "checked-tool", type: "tool" },
  { id: "unchecked-tool", isChecked: false, name: "unchecked-tool", type: "tool" },
];
...
describe("Tools", () => {
  ...
  it("shows the empty-state copy when the filter is on and nothing is selected", () => {
    const noneSelected = [
      { id: "a", isChecked: false, name: "a", type: "tool" as const },
      { id: "b", isChecked: false, name: "b", type: "tool" as const },
    ];
...
describe("Tools — toolkit filter pane", () => {
  const mixedTools: AgentTool[] = [
    { id: "native-tool", isChecked: false, name: "native-tool", type: "tool" },
    {
      hasConnection: true,
  ...
  beforeEach(() => {
    sharedServer.use(
      ...
      http.get(`${BASE_URL}/api/tool-providers/composio/toolkits`, () =>
        HttpResponse.json(composioToolkits),
      ),
  ...
  it("groups toolkits under their provider plus a Built-in group, all checked by default", async () => {
    ...
    expect(getByTestId("tools-toolkit-filter-checkbox-gmail").getAttribute("aria-checked")).toBe(
      "true",
    );
    expect(getByTestId("tools-toolkit-filter-checkbox-slack").getAttribute("aria-checked")).toBe(
      "true",
    );
    expect(
      getByTestId("tools-toolkit-filter-checkbox-__built-in__").getAttribute("aria-checked"),
    ).toBe("true");
  });
...
describe("Tools — integration rows without a connection", () => {
  beforeEach(() => {
    sharedServer.use(
      ...
      http.post(`${BASE_URL}/api/tool-providers/composio/authorize`, () =>
        HttpResponse.json({ authId: "auth_abc", url: "https://oauth.example/authorize" }),
      ),
          ...
          <FormProvider {...methods}>
            <AgentColorProvider agentId="agent_test">
              ...
              {onState && (
                <button
                  type="button"
                  data-testid="spy-form-state"
                  onClick={() => onState(methods.getValues())}
                >
                  spy
                </button>
              )}
  ...
  it("pins the new connection into the toolkit form field after successful OAuth when a tool is selected", async () => {
    ...
    sharedServer.use(
      http.post(`${BASE_URL}/api/tool-providers/composio/authorize`, () => {
        authorized = true;
        return HttpResponse.json({ authId: "auth_abc", url: "https://oauth.example/authorize" });
      }),
      http.get(`${BASE_URL}/api/tool-providers/composio/connections`, () =>
        HttpResponse.json({
          items: authorized ? [{ connectionId: "auth_abc", label: "work", status: "active" }] : [],
        }),
      ),
    );
    ...
    try {
      ...
      const { findByTestId, getByTestId } = render(
        <ConnectHarness
          ...
          defaultToolProviders={
            {
              composio: { connections: {}, tools: { GMAIL_FETCH_EMAILS: { toolkit: "gmail" } } },
      ...
      await waitFor(
        () => {
          ...
          const state = spy.mock.calls.at(-1)?.[0] as AgentBuilderEditFormValues | undefined;
...
describe("Tools — toolkit row connection management", () => {
  beforeEach(() => {
    ...
    sharedServer.use(
      ...
      http.get(`${BASE_URL}/api/tool-providers/composio/toolkits`, () =>
        HttpResponse.json({ data: [{ name: "Gmail", slug: "gmail" }] }),
      ),
    );
  });
    ...
    sharedServer.use(
      http.get(`${BASE_URL}/api/tool-providers/composio/connections`, async () => {
        ...
        return HttpResponse.json({
          items: [{ connectionId: "conn_only", label: "only", status: "active" }],
        });
      }),
    );
  ...
  it("renders connection-name badges under a connected tool card", async () => {
    ...
    const badges = await findByTestId(
      "tool-card-connections-integration-composio:GMAIL_FETCH_EMAILS",
    );
  ...
  it("does not render the connection-badges container when a connected tool has no labels", async () => {
    ...
    expect(
      queryByTestId("tool-card-connections-integration-composio:GMAIL_FETCH_EMAILS"),
    ).toBeNull();
  });
  ...
  it("shows a manage cog (not a Connect button) on the toolkit row when a connection exists", async () => {
    sharedServer.use(
      http.get(`${BASE_URL}/api/tool-providers/composio/connections`, () =>
        HttpResponse.json({
          items: [
            { connectionId: "conn_work", label: "work", status: "active" },
            { connectionId: "conn_pending", label: "pending", status: "pending" },
          ],
        }),
      ),
    );
  ...
  it("opening the manage cog renders the rename + disconnect dialog and disconnect is gated by an AlertDialog", async () => {
    ...
    await waitFor(() =>
      expect(queryByTestId("toolkit-manage-composio-gmail-disconnect-dialog")).toBeNull(),
    );
  });
  ...
  it("opening the manage cog with multiple active connections shows a list, drilling into one to manage it", async () => {
    let items = [
      { connectionId: "conn_work", label: "work", status: "active" },
      { connectionId: "conn_personal", label: "personal", status: "active" },
    ];
    sharedServer.use(
      http.get(`${BASE_URL}/api/tool-providers/composio/connections`, () =>
        HttpResponse.json({ items }),
    ...
    await findByTestId("toolkit-manage-composio-gmail-list");
    await waitFor(() =>
      expect(queryByTestId("toolkit-manage-composio-gmail-list-item-conn_work")).toBeNull(),
    );
    getByTestId("toolkit-manage-composio-gmail-list-item-conn_personal");
  });
  ...
  it("pins the toolkit active connection into the form while a tool in the toolkit is selected", async () => {
    ...
    await waitFor(() => {
      ...
      const pinned = (
        state as unknown as {
          toolProviders: {
            composio?: { connections?: { gmail?: { connectionId: string }[] } };
          };
        }
      ).toolProviders.composio?.connections?.gmail;
      expect(pinned).toEqual([
        expect.objectContaining({ connectionId: "conn_only", kind: "author", toolkit: "gmail" }),
      ]);
    });
  });
  ...
  it("pins every active connection while a tool in the toolkit is selected", async () => {
    ...
    await waitFor(() => {
      ...
      expect(pinned?.map((c) => c.connectionId).toSorted()).toEqual(["conn_personal", "conn_work"]);
    });
  });
  ...
  it("renders the backend provider icon on the rename form when the toolkit has one", async () => {
    ...
    const form = input.closest(
      '[data-testid="toolkit-manage-composio-gmail-dialog"]',
    ) as HTMLElement;
    ...
    sharedServer.use(
      http.get(`${BASE_URL}/api/tool-providers`, () =>
        HttpResponse.json({
          providers: [
            {
              capabilities: { multipleConnectionsPerToolkit: false },
              id: "composio",
              name: "Composio",
            },
          ],

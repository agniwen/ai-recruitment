...
describe("AgentBuilderStarter", () => {
  beforeEach(() => {
    ...
    server.use(
      ...
      http.get(`${BASE_URL}/api/auth/capabilities`, () =>
        HttpResponse.json({ enabled: true, login: null }),
      ),
    );
  });
  ...
  it("eagerly creates the agent then navigates to its edit page with the user message", async () => {
    ...
    expect(capturedBody.model).toEqual({ name: "gemini-2.5-flash", provider: "google" });
  ...
  it("truncates long prompts to 20 chars + ellipsis when generating the temp name", async () => {
    ...
    await waitFor(() =>
      expect((getByTestId("agent-builder-starter-submit") as HTMLButtonElement).disabled).toBe(
        false,
      ),
    );
  ...
  it("prefers the admin-configured modelPolicy default over the first allowed model (PLTFRM-1017)", async () => {
      ...
      http.get(`${BASE_URL}/api/editor/builder/settings`, () =>
        HttpResponse.json({
          ...
          modelPolicy: {
            active: true,
            allowed: [
              { modelId: ALLOWED_MODEL_ID, provider: ALLOWED_MODEL_PROVIDER },
              { provider: DEFAULT_MODEL_PROVIDER },
            ],
            default: { modelId: DEFAULT_MODEL_ID, provider: DEFAULT_MODEL_PROVIDER },
            pickerVisible: true,
          },
        }),
      ),
    ...
    fireEvent.change(getByTestId("agent-builder-starter-input"), {
      target: { value: "standup bot" },
    });
    await waitFor(() =>
      expect((getByTestId("agent-builder-starter-submit") as HTMLButtonElement).disabled).toBe(
        false,

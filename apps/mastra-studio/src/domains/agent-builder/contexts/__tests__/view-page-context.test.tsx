  ...
  return (
    <div>
      ...
      <button
        ...
        onClick={() => ctx.onModeToggle?.()}
        disabled={!ctx.onModeToggle}
      >
        toggle
      </button>
    </div>
...
describe("ViewPageProvider", () => {
  ...
  describe("when the viewer is the owner", () => {
    it("exposes a mode-toggle that navigates to the edit page", () => {
      ...
      expect(navigateMock).toHaveBeenLastCalledWith("/agent-builder/agents/agent-1/edit", {
        viewTransition: true,
      });
    });
  });

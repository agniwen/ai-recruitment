...
const textThenFinishStream = (text: string) =>
  new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      controller.enqueue(
        encoder.encode(
          `data: ${JSON.stringify({ payload: { id: "text-1", text }, runId: "run-1", type: "text-delta" })}\n\n`,
        ),
      );
      controller.enqueue(
...
const sseResponse = (stream: ReadableStream<Uint8Array>) =>
  new HttpResponse(stream, { headers: { "content-type": "text/event-stream" }, status: 200 });
...
describe("StreamChatProvider", () => {
  ...
  describe("when consumers subscribe to split contexts", () => {
    it("does not re-render running subscribers while only the message list changes", async () => {
      ...
      server.use(
        ...baseHandlers(),
        http.post(`${BASE_URL}/api/agents/a/stream`, () =>
          sseResponse(textThenFinishStream("streamed reply")),
        ),
      );
      ...
      // messages subscriber.
      await waitFor(() =>
        expect(view.getByTestId("assistant-text").textContent).toContain("streamed reply"),
      );
  ...
  describe("when initialUserMessage is provided on mount", () => {
    it("dispatches the starter message exactly once", async () => {
      ...
      expect(captured[0].body.messages?.at(-1)).toMatchObject({
        content: [{ text: "kickoff", type: "text" }],
        role: "user",
      });
    });
  });

...
describe("AgentColorProvider", () => {
  ...
  it("derives background, foreground, and tint colors from the agentId", () => {
    ...
    expect(consumer.dataset.bg).toBe(stringToColor("agent_123"));
    expect(consumer.dataset.fg).toBe(stringToColor("agent_123", 20));
    expect(consumer.dataset.tint).toBe(stringToColor("agent_123", 50));
    // Background uses lightness 90%, foreground uses lightness 20%, tint uses lightness 50%.
    expect(consumer.dataset.bg).toMatch(/hsl\(-?\d+, 100%, 90%\)/);
    expect(consumer.dataset.fg).toMatch(/hsl\(-?\d+, 100%, 20%\)/);
    expect(consumer.dataset.tint).toMatch(/hsl\(-?\d+, 100%, 50%\)/);
  });
  ...
  it("produces different colors for different agentIds", () => {
    ...
    const consumer = getByTestId("consumer");
    expect(consumer.dataset.bg).toBe(stringToColor("alpha"));
    ...
    expect(consumer.dataset.bg).toBe(stringToColor("omega"));
  });
});

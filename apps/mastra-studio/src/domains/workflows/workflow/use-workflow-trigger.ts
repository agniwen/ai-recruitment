...
export function useSuspendedSteps(
  streamResult: WorkflowRunStreamResult | null,
  runId: string,
): SuspendedStep[] {
  return useMemo(
    () =>
      Object.entries(streamResult?.steps || {})
...
function useWorkflowStepGraphInfo(stepGraph: GetWorkflowResponse["stepGraph"] | undefined) {
  return useMemo(() => {
    ...
    const stepNodesInOrder = nodes.flatMap((node) => {
      if (
        node.type !== WORKFLOW_STEP_NODE_TYPE ||
        node.data.nodeRole === "condition" ||
        !node.data.stepId
      ) {
        return [];
      }
...
export function useWaitingStepKey(): string | undefined {
  ...
  const { stepNodesInOrder, stepsFlow, stepSuccessors, conditionalStepIds } =
  ...
  const isStepSuccess = useCallback(
    (stepId: string) => steps?.[stepId]?.status === "success",
    [steps],
  );
  ...
  const isStepResolved = useCallback(
    (stepId: string) =>
      steps?.[stepId]?.status === "success" || steps?.[stepId]?.status === "skipped",
    [steps],
  );
  const isStepBypassed = useCallback(
    (stepId: string) =>
      isBranchArmBypassed({ conditionalStepIds, isStepSuccess, stepId, stepSuccessors, stepsFlow }),
  ...
  return useMemo(
    () =>
      isPaused
        ? selectNextStepKey({ isStepBypassed, isStepSuccess: isStepResolved, stepNodesInOrder })
        : undefined,
...
export function useSuspendedStepKey(): string | undefined {
  ...
  return useMemo(() => {
    const entry = Object.entries(result?.steps || {}).find(
      ([_, { status }]) => status === "suspended",
    );
    return entry?.[0];
  }, [result?.steps]);
}
...
export function useNextPerStep() {
  ...
  const stepPayload = useMemo(() => {
    const input = buildNextStepInput({ isStepBypassed, nextStepKey, steps, stepsFlow });
    if (input) {
      return input;
    }
...
export function useResumeWorkflow() {
  const { workflowId, workflow, createWorkflowRun, resumeWorkflow } =
    useContext(WorkflowRunContext);
  const requestContext = useMergedRequestContext();
  ...
  return useCallback(
    async (step: ResumeStepParams) => {
      ...
      const run = await createWorkflowRun({ prevRunId, workflowId });

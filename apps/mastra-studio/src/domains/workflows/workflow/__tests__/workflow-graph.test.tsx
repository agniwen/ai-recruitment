...
const twoNodes = [
  {
    data: { label: "step-a", stepId: "step-a" },
    id: "step-a",
    measured: { height: 120, width: 300 },
    position: { x: 40, y: 80 },
  },
  {
    data: { label: "step-b", stepId: "step-b" },
    id: "step-b",
    measured: { height: 120, width: 300 },
    position: { x: 440, y: 80 },
  },
...
describe("WorkflowGraph", () => {
  it("focuses and zooms the graph viewport when a workflow step is selected", async () => {
    ...
    await waitFor(() => {
      expect(reactFlowViewport.setCenter).toHaveBeenCalledWith(190, 140, {
        duration: 300,
        zoom: 1,
      });
    });
  ...
  it("auto-focuses the step a paused run is waiting on, without any selection", async () => {
    ...
    await waitFor(() => {
      expect(reactFlowViewport.setCenter).toHaveBeenCalledWith(590, 140, {
        duration: 300,
        zoom: 1,
      });
    });
  ...
  it("retries centering on the waiting step once nodes lay out after the first paint", async () => {
    ...
    reactFlowViewport.getNodes.mockImplementation(() =>
      nodesLaidOut ? (twoNodes as never) : ([] as never),
    );
    ...
    act(() => {
      reactFlowControl.onNodesChange?.([
        { id: "step-a", position: { x: 1, y: 0 }, type: "position" },
      ]);
    });

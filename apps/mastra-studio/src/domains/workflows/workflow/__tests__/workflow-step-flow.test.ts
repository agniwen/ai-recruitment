...
describe("collectGraphStepFlags", () => {
  it("collects conditional arm ids but not parallel arm ids", () => {
    const stepGraph = [
      {
        steps: [
          { step: { id: "p1" }, type: "step" },
          { step: { id: "p2" }, type: "step" },
        ],
        type: "parallel",
      },
      {
  ...
  it("flags nested workflow steps by component", () => {
    const stepGraph = [
      { step: { component: "STEP", id: "plain" }, type: "step" },
  ...
  it("returns empty sets for an undefined graph", () => {
    const { conditionalStepIds, nestedWorkflowStepIds } = collectGraphStepFlags();
...
describe("isBranchArmBypassed", () => {
  ...
  const stepSuccessors = { long: ["join"], short: ["join"] };
...
describe("selectNextStepKey", () => {
  ...
  it("selects the first step that has not yet succeeded", () => {
    ...
    expect(
      selectNextStepKey({ isStepBypassed: noneBypassed, isStepSuccess, stepNodesInOrder }),
    ).toBe("b");
  });
  ...
  it("skips a bypassed branch arm and selects the next runnable step", () => {
    ...
    expect(selectNextStepKey({ isStepBypassed, isStepSuccess, stepNodesInOrder })).toBe("c");
  });

  it("returns undefined when every step has succeeded", () => {
    expect(
      selectNextStepKey({
        isStepBypassed: noneBypassed,
        isStepSuccess: () => true,
        stepNodesInOrder,
      }),
    ).toBeUndefined();
...
describe("isLastRunnableStep", () => {
  ...
  it("treats bypassed later steps as not needing to run", () => {
    ...
    expect(
      isLastRunnableStep({ isStepBypassed, isStepSuccess, nextStepKey: "b", stepNodesInOrder }),
    ).toBe(true);
  });
...
describe("buildNextStepInput", () => {
  it("passes a single predecessor output directly", () => {
    const stepsFlow = { b: ["a"] };
    const steps = { a: { output: { value: 1 }, status: "success" } };

    expect(buildNextStepInput({ nextStepKey: "b", steps, stepsFlow })).toEqual({
      hasMultiSteps: false,
      input: { value: 1 },
    });
  });
  ...
  it("builds a keyed map of outputs for a join with multiple predecessors", () => {
    ...
    const steps = {
      a: { output: { x: 1 }, status: "success" },
      b: { output: { y: 2 }, status: "success" },
    };

    expect(buildNextStepInput({ nextStepKey: "join", steps, stepsFlow })).toEqual({
      hasMultiSteps: true,
      input: { a: { x: 1 }, b: { y: 2 } },
    });
  });
  ...
  it("returns undefined when the single predecessor has not succeeded", () => {
    ...
    expect(buildNextStepInput({ nextStepKey: "b", steps, stepsFlow })).toBeUndefined();
  });
  ...
  it("returns undefined for a join when only some predecessors have succeeded", () => {
    ...
    expect(buildNextStepInput({ nextStepKey: "join", steps, stepsFlow })).toBeUndefined();
  });
  ...
  it("builds a join from the taken arm only when the other arm is a bypassed dead branch", () => {
    ...
    const steps = { taken: { output: { x: 1 }, status: "success" } };
    const isStepBypassed = (stepId: string) => stepId === "dead-arm";

    expect(buildNextStepInput({ isStepBypassed, nextStepKey: "join", steps, stepsFlow })).toEqual({
      hasMultiSteps: true,
      input: { taken: { x: 1 } },
    });
  });
  ...
  it("returns undefined when the step has no predecessor or no next step", () => {
    expect(buildNextStepInput({ nextStepKey: "orphan", steps: {}, stepsFlow: {} })).toBeUndefined();
    expect(
      buildNextStepInput({ nextStepKey: undefined, steps: {}, stepsFlow: {} }),
    ).toBeUndefined();
  });
});

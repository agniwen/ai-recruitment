...
function serveWorkflowRun(
  workflowId: string,
  workflow: GetWorkflowResponse,
  run: GetWorkflowRunByIdResponse,
) {
  server.use(
    http.get(`${BASE_URL}/api/workflows/${workflowId}`, () => HttpResponse.json(workflow)),
    http.get(`${BASE_URL}/api/workflows/${workflowId}/runs/${run.runId}`, () =>
        ...
        <ul data-testid="suspended">
          {suspended.map((step) => (
            <li
              ...
              data-payload={JSON.stringify(step.suspendPayload)}
            >
              {step.stepId}
            </li>
          ))}
  ...
  describe("when the run has a suspended step", () => {
    it("surfaces only the suspended step with its payload and run id", async () => {
      ...
      const items = await waitFor(() => {
        ...
        if (found.length === 0) {
          throw new Error("no suspended steps yet");
        }
        return found;
      });

      expect(items).toHaveLength(1);
      expect(items[0].dataset.stepId).toBe("transform");
      expect(items[0].dataset.payload).toBe(JSON.stringify({ question: "continue?" }));
    });
  });

          ...
          {type === "else" ? null : (
            <CollapsibleContent className="flex flex-col gap-2 pb-2">
              {conditions.map((condition, index) => {
                ...
                const { icon: ConjIconComponent, color: conjColor } =
                  getConditionIconAndColor(conjType);
                const conjBadge =
                  index === 0 ? null : (
                    <Badge
                      icon={
                            ...
                            >
                              {({ className, style, tokens, getLineProps, getTokenProps }) => (
                                ...
                                >
                                  {tokens.map((line, i) => (
                                    <div key={i} {...getLineProps({ line })}>
                                      <span className="inline-block mr-2 text-neutral3">
                                        {i + 1}
                                      </span>
                                      {line.map((token, key) => (
                                        <span key={key} {...getTokenProps({ token })} />
                    ...
                    {condition.ref?.step ? (
                      <div className="flex items-center gap-1">
                        ...
                        <Txt variant="ui-xs" className=" text-neutral3 flex-1">
                          {(condition.ref.step as any).id || condition.ref.step}'s{" "}
                          {condition.ref.path}{" "}

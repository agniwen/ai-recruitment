...
const getDisplayStatus = (
  step?: Step,
): { displayStatus: WorkflowCardDisplayStatus; isTripwire: boolean } => {
...
const WorkflowStepCard = ({
  ...
  const mapConfig =
    data.mapConfig ?? ("step" in data.workflowStep ? data.workflowStep.step?.mapConfig : undefined);
  const stepGraph =
    data.stepGraph ??
    ...
    <WorkflowStepCardView
      ...
      actionBar={
        <WorkflowStepActionBar
          ...
          mapConfig={mapConfig}
          onShowNestedGraph={
            stepGraph ? () => showNestedGraph({ fullStep: fullLabel, label, stepGraph }) : undefined
          }
...
const WorkflowConditionCard = ({ data }: { data: WorkflowStepNodeData }) => {
  ...
  const [dialogCondition, setDialogCondition] = useState<
    WorkflowConditionCodeCondition | undefined
  >();
  const { steps } = useCurrentRun();

...
function stubRuns(response: ListWorkflowRunsResponse) {
  server.use(
    http.get(`${BASE_URL}/api/workflows/${WORKFLOW_ID}/runs`, () => HttpResponse.json(response)),
  );
}
...
describe("WorkflowRecentRuns", () => {
  ...
  it("shows an empty state and no run rows when there are no runs", async () => {
    ...
    expect(
      screen.getByText("Your run history will appear here once you run the workflow"),
    ).not.toBeNull();
  });

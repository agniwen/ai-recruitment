...
function RunWorkflowSidebar({ runId, observeWorkflowStream, ...props }: RunWorkflowSidebarProps) {
  return (
    <WorkflowRunDetail {...props} runId={runId} observeWorkflowStream={observeWorkflowStream} />
  );
}
...
export function WorkflowInformation({ workflowId, initialRunId }: WorkflowInformationProps) {
  ...
  const isCurrentRunFinished = ["success", "failed", "canceled", "bailed"].includes(
    streamResult?.status ?? "",
  );
  ...
  return (
    ...
    >
      <WorkflowInformationTopSection
        newRunButton={
          showNewRunButton ? (
            <NewWorkflowRunButton workflowId={workflowId} onClick={resetToNewRun} />
          ) : undefined
        }
      >
        {initialRunId ? (
      ...
      <RecentWorkflowRunsSection
        ...
        activeRunId={initialRunId || runId || contextRunId}
      />
    </div>
  );
}

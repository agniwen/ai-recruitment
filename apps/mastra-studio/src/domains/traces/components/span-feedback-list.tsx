...
interface SpanFeedbackListProps {
...
export function SpanFeedbackList({
  feedbackData,
  isLoadingFeedbackData,
  onPageChange,
}: SpanFeedbackListProps) {
  ...
  const toNext =
    selectedIndex >= 0 && selectedIndex < feedbackItems.length - 1
      ...
      : undefined;
  const toPrevious =
    selectedIndex > 0 ? () => setSelectedFeedback(feedbackItems[selectedIndex - 1]) : undefined;

...
vi.mock("react-resizable-panels", () => ({
  ...
  Panel: ({
    ...
    children: ReactNode;
    panelRef?: Ref<{
      getSize: () => { inPixels: number; asPercentage: number };
...
describe("resizable service layouts", () => {
  ...
  it("expands the single left slot to 50% when observational memory opens and restores it on close", async () => {
    ...
    const mainPanel = screen.getByTestId("panel-main-slot");
    expect(
      leftPanel.compareDocumentPosition(mainPanel) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    ...
    expect(screen.getByTestId("panel-left-slot").dataset.maxSize).toBe("50%");
  ...
  it("renders a resizable right slot when rightSlot is provided on desktop", () => {
    render(
      <AgentLayout
        ...
        rightSlot={<div>memory studio</div>}
      >
        <div>chat</div>
      </AgentLayout>,
    );
  ...
  it("keeps the workflow panel group shrinkable when side slots are present", () => {
    render(
      <WorkflowLayout
        ...
        leftSlot={<div>runs</div>}
        rightSlot={<div>workflow information</div>}
      >

...
describe("ComposerModelSettings", () => {
  ...
  it("renders the popover content once data resolves and exposes the chat method controls", async () => {
    ...
    expect(document.querySelector("#generate")).not.toBeNull();
    expect(document.querySelector("#streamSubscription")).not.toBeNull();
    expect(document.querySelector("#stream")).not.toBeNull();
    expect(document.querySelector("#generateLegacy")).toBeNull();
    expect(document.querySelector("#streamLegacy")).toBeNull();
  });
  ...
  it("persists legacy stream as an explicit no-subscription fallback", async () => {
    ...
    const legacyStream = document.querySelector("#stream");
    ...
    const stored = JSON.parse(
      window.localStorage.getItem(`mastra-agent-store-${AGENT_ID}`) ?? "{}",
    );
  ...
  it("falls back to stream and disables stream subscription for agents without memory support", async () => {
    server.use(
      http.get(`${BASE_URL}/api/agents/${AGENT_ID}`, () =>
        HttpResponse.json({ ...v2Agent, supportsMemory: false }),
      ),
      http.get(`${BASE_URL}/api/memory/status`, () => HttpResponse.json(memoryDisabled)),
      http.get(`${BASE_URL}/api/auth/capabilities`, () => HttpResponse.json({ enabled: false })),
    );
    ...
    expect((document.querySelector("#stream") as HTMLInputElement | null)?.checked).toBe(true);
    expect(
      (document.querySelector("#streamSubscription") as HTMLInputElement | null)?.disabled,
    ).toBe(true);
  });

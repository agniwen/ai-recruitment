...
afterEach(() => {
  ...
  delete (window as typeof window & { MASTRA_REQUEST_CONTEXT_PRESETS?: string })
...
describe("ComposerRunOptions", () => {
  ...
  it("falls back to the free-form request context editor when the agent has no schema", async () => {
    ...
    expect(
      await screen.findByText("Request Context (JSON)", undefined, { timeout: 10_000 }),
    ).not.toBeNull();
  }, 15_000);

  it("disables free-form request context save until the draft changes and supports revert", async () => {
    (
      window as typeof window & { MASTRA_REQUEST_CONTEXT_PRESETS?: string }
    ).MASTRA_REQUEST_CONTEXT_PRESETS = JSON.stringify({
      French: { locale: "fr" },
    });

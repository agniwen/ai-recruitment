...
describe("AgentChannels MSW integration", () => {
  it("renders a connected platform when installations are active", async () => {
    server.use(
      ...
      http.get(`${BASE_URL}/api/channels/slack/installations`, () =>
        HttpResponse.json(slackInstallations),
      ),
    );
  ...
  it("filters platform rows by the search input", async () => {
    server.use(
      ...
      http.get(`${BASE_URL}/api/channels/platforms`, () =>
        HttpResponse.json(slackAndDiscordPlatforms),
      ),
      http.get(`${BASE_URL}/api/channels/slack/installations`, () =>
        HttpResponse.json(slackInstallations),
      ),
      http.get(`${BASE_URL}/api/channels/discord/installations`, () =>
        HttpResponse.json(noSlackInstallations),
      ),
    );
    ...
    fireEvent.change(screen.getByPlaceholderText("Filter by platform name"), {
      target: { value: "slack" },
    });

...
const memoryOn: GetMemoryStatusResponse = { memoryType: "local", result: true };
...
function registerFooterHandlers({
  ...
  server.use(
    ...
    http.get(`${BASE_URL}/api/system/packages`, () =>
      HttpResponse.json({ ...systemPackages, cmsEnabled }),
    ),
...
describe("AgentCapabilitiesFooter", () => {
  ...
  it("marks the editor locked and skips the versions fetch when the agent disables editing", async () => {
    registerFooterHandlers({ agent: { ...baseAgent, editor: false }, cmsEnabled: true });
    renderFooter();
  ...
  it.each([
    {
      label: "gateway",
      memory: { memoryType: "gateway", result: true } as GetMemoryStatusResponse,
      status: "Gateway",
    },

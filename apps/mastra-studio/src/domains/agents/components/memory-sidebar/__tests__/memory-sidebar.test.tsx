...
import {
  ...
  useMemoryTimeline,
} from "@/domains/agents/context/memory-timeline-context";
...
function registerMemoryHandlers() {
  server.use(
    http.get(`${BASE_URL}/api/auth/capabilities`, () =>
      HttpResponse.json(readOnlyAuthCapabilities),
    ),
    ...
    http.get(`${BASE_URL}/api/memory/threads/:threadId/working-memory`, () =>
      HttpResponse.json({ source: "thread", workingMemory: null }),
    ),
  );
}
...
describe("MemorySidebar", () => {
  it("renders the Memory card as an overlay above the thread list by default", async () => {
    ...
    expect(card.closest('[data-testid="memory-sidebar-overlay"]')?.className).toContain(
      "rounded-xl",
    );
  ...
  it("renders the timeline panel context window from the OM record (source of truth), not message markers", async () => {
    server.use(
      http.get(`${BASE_URL}/api/memory/config`, () =>
        HttpResponse.json(observationalMemoryConfigWithThresholds),
      ),
      http.get(`${BASE_URL}/api/memory/observational-memory`, () =>
        HttpResponse.json(observationalMemoryWithRecord),
      ),
      http.get(`${BASE_URL}/api/memory/threads/${THREAD_ID}/messages`, () =>
        HttpResponse.json(threadMessages),
      ),
    );
  ...
  it("filters the observation list to the selected zoom range", async () => {
    ...
    server.use(
      http.get(`${BASE_URL}/api/memory/config`, () => HttpResponse.json(observationalMemoryConfig)),
      http.get(`${BASE_URL}/api/memory/observational-memory`, () =>
        HttpResponse.json(observationalMemoryTwoRecords),
      ),
      http.get(`${BASE_URL}/api/memory/threads/${THREAD_ID}/messages`, () =>
        HttpResponse.json(threadMessagesSpan),
      ),
    );
    ...
    track.getBoundingClientRect = () => ({ height: 24, left: 0, top: 0, width: 100 }) as DOMRect;

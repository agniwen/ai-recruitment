...
export function BrowserSessionProvider({
  ...
  const { data: probe } = useBrowserSessionProbe({ agentId, enabled, threadId });
  ...
  const closeBrowser = useCallback(async () => {
    if (closeBrowserMutation.isPending || !agentId) {
      return;
    }
  ...
  return (
    <BrowserSessionContext.Provider value={value}>
      <BrowserFrameStoreContext.Provider value={frameStoreRef.current}>
        {children}
      </BrowserFrameStoreContext.Provider>

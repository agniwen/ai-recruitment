...
describe("normalizeToolsToRecord", () => {
  it("returns empty record for undefined", () => {
    expect(normalizeToolsToRecord()).toEqual({});
  });
...
describe("splitModelId", () => {
  it('splits "provider/name"', () => {
    expect(splitModelId("openai/gpt-4")).toEqual({ name: "gpt-4", provider: "openai" });
  });

  it("handles model names with slashes", () => {
    expect(splitModelId("google/models/gemini-2.0")).toEqual({
...
describe("joinModelId", () => {
  it("joins provider and name", () => {
    expect(joinModelId({ name: "gpt-4", provider: "openai" })).toBe("openai/gpt-4");
  });
  ...
  it("returns undefined for undefined input", () => {
    expect(joinModelId()).toBeUndefined();
  });
});
...
describe("normalizeIntegrationToolsToRecord", () => {
  it("returns empty record for undefined", () => {
    expect(normalizeIntegrationToolsToRecord()).toEqual({});
  });
...
describe("mapInstructionBlocksToApi", () => {
  it("maps inline blocks stripping the id", () => {
    const blocks = [
      { content: "Hello", id: "1", type: "prompt_block" as const },
      { content: "World", id: "2", rules: undefined, type: "prompt_block" as const },
    ];
    expect(mapInstructionBlocksToApi(blocks)).toEqual([
      { content: "Hello", rules: undefined, type: "prompt_block" },
      { content: "World", rules: undefined, type: "prompt_block" },
    ]);
  });

  it("maps ref blocks to prompt_block_ref with id", () => {
    const blocks = [
      { id: "ui-1", promptBlockId: "saved-block-123", type: "prompt_block_ref" as const },
    ];
    expect(mapInstructionBlocksToApi(blocks)).toEqual([
      { id: "saved-block-123", type: "prompt_block_ref" },
    ]);
  });

  it("maps mixed inline and ref blocks", () => {
    const blocks = [
      { content: "Inline", id: "1", type: "prompt_block" as const },
      { id: "2", promptBlockId: "ref-456", type: "prompt_block_ref" as const },
    ];
    expect(mapInstructionBlocksToApi(blocks)).toEqual([
      { content: "Inline", rules: undefined, type: "prompt_block" },
      { id: "ref-456", type: "prompt_block_ref" },
    ]);
  });

  it("returns empty array for undefined", () => {
    expect(mapInstructionBlocksToApi()).toEqual([]);
  });
});
...
describe("mapInstructionBlocksFromApi", () => {
  it("parses array of prompt_block instructions", () => {
    const raw = [
      { content: "A", type: "prompt_block" as const },
      { content: "B", type: "prompt_block" as const },
    ];
  ...
  it("parses prompt_block_ref instructions", () => {
    const raw = [
      { content: "Inline", type: "prompt_block" as const },
      { id: "ref-123", type: "prompt_block_ref" as const },
    ];
  ...
  it("filters out text type blocks", () => {
    const raw = [
      { content: "Keep", type: "prompt_block" as const },
      { content: "Skip", type: "text" as const },
    ];
    const { instructionBlocks } = mapInstructionBlocksFromApi(raw);
  ...
  it("normalizes legacy { content, role } object in prompt_block content", () => {
    const raw = [
      {
        content: { content: "You are a chef.", role: "system" } as unknown as string,
  ...
  it("handles CoreSystemMessage object as top-level instructions", () => {
    ...
    const raw = { content: "You are a chef.", role: "system" } as unknown as string;
    const { instructionsString, instructionBlocks } = mapInstructionBlocksFromApi(raw);
    expect(instructionsString).toBe("You are a chef.");
    expect(instructionBlocks).toHaveLength(1);
    expect(instructionBlocks[0].type).toBe("prompt_block");
  ...
  it("handles CoreSystemMessage[] array as top-level instructions", () => {
    // When agent.instructions is [{ role: 'system', content: '...' }, ...]
    const raw = [
      { content: "First instruction.", role: "system" },
      { content: "Second instruction.", role: "system" },
...
describe("mapScorersToApi", () => {
  it("returns undefined for undefined", () => {
    expect(mapScorersToApi()).toBeUndefined();
  });
  ...
  it("maps scorers with sampling", () => {
    const input = {
      accuracy: {
        ...
        sampling: { rate: 0.5, type: "ratio" as const },
      },
    };
...
describe("buildObservationalMemoryForApi", () => {
  it("returns undefined when disabled", () => {
    expect(buildObservationalMemoryForApi()).toBeUndefined();
...
describe("parseObservationalMemoryFromApi", () => {
  it("returns undefined for undefined", () => {
    expect(parseObservationalMemoryFromApi()).toBeUndefined();
  });
  ...
  it("parses full config", () => {
    ...
    expect(result.enabled).toBe(true);
    expect(result.model).toEqual({ name: "gpt-4", provider: "openai" });
    expect(result.scope).toBe("thread");
    expect(result.shareTokenBudget).toBe(true);
    expect(result.observation!.model).toEqual({ name: "gpt-4o", provider: "openai" });
    expect(result.observation!.messageTokens).toBe(1000);
    expect(result.reflection!.model).toEqual({ name: "claude-3", provider: "anthropic" });

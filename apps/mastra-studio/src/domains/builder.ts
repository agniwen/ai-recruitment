...
} from "./agent-builder/hooks/use-builder-settings";

const modelAllowed = (policy: BuilderModelPolicy, provider: string, modelId?: string) => {
  if (!policy.active || !policy.allowed?.length) {
    return true;
  }
  return policy.allowed.some((entry) => {
    if (!providerMatches(provider, entry.provider)) {
      return false;
    }
    if (!entry.modelId) {
      return true;
...
export function useBuilderFilteredModels(models: ModelInfo[], policy: BuilderModelPolicy) {
  return useMemo(
    () => models.filter((model) => modelAllowed(policy, model.provider, model.model)),
    [models, policy],
  );
}

...
vi.mock("@mastra/playground-ui/components/Dialog", () => {
  const Dialog = ({ open, children }: PropsWithChildren<{ open: boolean }>) =>
...
describe("AddItemDialog", () => {
  ...
  it("rejects non-array Tool Mocks JSON before making a request", async () => {
    ...
    await waitFor(() =>
      expect(toast.error).toHaveBeenCalledWith("Tool Mocks must be a JSON array"),
    );
  ...
  it("renders server-side Tool Mocks validation errors inline", async () => {
        ...
        HttpResponse.json(
          {
            ...
            errors: [{ message: "Required", path: "0.output" }],
            field: "toolMocks",
          },
          { status: 400 },
        ),
    ...
    fireEvent.change(input, { target: { value: '{"city":"Seattle"}' } });
    fireEvent.change(toolMocks, {
      target: { value: JSON.stringify([{ args: {}, toolName: "getWeather" }]) },
    });

...
export function DatasetsList({
  ...
  const filteredData = useMemo(() => {
    ...
    return enrichedDatasets.filter((ds) => {
      ...
      const matchesTag =
        tagFilter === "all" ||
        (Array.isArray(ds.tags) && (ds.tags as string[]).includes(tagFilter));
        ...
        return (
          <EntityList.RowWrapper key={ds.id}>
            ...
            >
              ...
              <EntityList.Cell>
                {tags.length > 0 ? (
                  ...
                  >
                    ...
                    {tags.length > 2 && (
                      <span className="shrink-0 text-[10px] text-neutral2">+{tags.length - 2}</span>
                    )}
                  </div>
                ) : (
              ...
              {rowLayout.showExperimentsPlaceholder ? (
                <EntityList.Cell className="justify-center" />
              ) : null}
              {rowLayout.showReviewPlaceholderInLink ? (
                <EntityList.Cell className="justify-center" />
              ) : null}
            </EntityList.RowLink>

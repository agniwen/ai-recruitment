...
const captureSignIn = (responseBody: unknown, ok = true): CapturedRequest => {
  const captured: CapturedRequest = { count: 0, headers: new Headers(), url: "" };
  server.use(
...
const signIn = async (providerProps: ProviderProps) => {
  const { result } = renderHook(() => useCredentialsLogin(), {
...
const signUp = async (providerProps: ProviderProps) => {
  const { result } = renderHook(() => useCredentialsSignUp(), {
    wrapper: makeWrapper(providerProps),
  });
  await act(async () => {
    result.current.mutate({ email: "a@b.c", name: "A", password: "pw" });
  });
  await waitFor(() => expect(result.current.isPending).toBe(false));
  return result;
};
...
describe("useCredentialsLogin", () => {
  describe("when the client has a custom apiPrefix", () => {
    it("targets the sign-in endpoint under that prefix", async () => {
      const captured = captureSignIn({ user: { email: "a@b.c", id: "1" } });
  ...
  describe("when the client carries custom headers", () => {
    it("forwards them on the sign-in request", async () => {
      ...
      await signIn({ headers: { Authorization: "Bearer dev-token", "x-tenant-id": "tenant-123" } });
...
describe("useCredentialsSignUp", () => {
  describe("when the client has a custom apiPrefix", () => {
    it("targets the sign-up endpoint under that prefix", async () => {
      const captured = captureSignUp({ user: { email: "a@b.c", id: "1" } });
  ...
  describe("when the client carries custom headers", () => {
    it("forwards them on the sign-up request", async () => {
      ...
      await signUp({ headers: { Authorization: "Bearer dev-token", "x-tenant-id": "tenant-123" } });

...
export interface RoleImpersonationState {
...
export function RoleImpersonationProvider({ children }: { children: ReactNode }) {
  ...
  const [impersonatedRole, setImpersonatedRole] = useState<{ id: string; name: string } | null>(
    null,
  );
  ...
  >({
    mutationFn: async ({ role }) => {
      ...
      return { permissions: data.permissions, role };
    },
  ...
  return (
    <RoleImpersonationContext.Provider value={value}>{children}</RoleImpersonationContext.Provider>
  );
}

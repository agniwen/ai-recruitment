...
const VoiceHarness = ({ onCallStarted }: { onCallStarted?: () => void }) => {
  const voiceCall = useVoiceCall({ agentId: "support", onCallStarted, threadId: "thread-1" });
  return (
    <>
  ...
  it("starts a call: fetches connection details, joins the room, and enables the mic", async () => {
    ...
    await waitFor(() =>
      expect(invalidateSpy).toHaveBeenCalledWith({
        queryKey: ["memory", "threads", "support", "support"],
      }),
    );
  ...
  it("shows agent state changes and live captions", async () => {
    server.use(
      http.post(`${BASE_URL}/voice/livekit/connection-details`, () =>
        HttpResponse.json(connectionDetails),
      ),
    );
    ...
    act(() => {
      room.handlers.get("participantAttributesChanged")?.(
        ...
        { attributes: {}, identity: "agent-AJ_x" },
      );
    });
    ...
    expect((await screen.findByTestId("voice-caption-user")).textContent).toBe(
      "What is the weather?",
    );
    expect((await screen.findByTestId("voice-caption-agent")).textContent).toBe(
    ...
    await waitFor(
      () =>
        expect(invalidateSpy).toHaveBeenCalledWith({
          queryKey: ["memory", "messages", "thread-1"],
        }),
      {
        timeout: 2000,
      },

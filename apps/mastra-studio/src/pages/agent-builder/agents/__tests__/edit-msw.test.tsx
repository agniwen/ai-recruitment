...
const baseHandlers = (overrides?: Partial<typeof storedAgent>) => [
  ...
  http.get(`${BASE_URL}/api/stored/agents/agent-123`, () =>
    HttpResponse.json({ ...storedAgent, ...overrides }),
  ),
...
describe("AgentBuilderAgentEdit MSW integration — visibility immediate-persist", () => {
  ...
  it("keeps the Add to library button enabled while a stream is running", async () => {
    ...
    const addButton = await screen.findByTestId("agent-builder-visibility-add", undefined, {
      timeout: 10_000,
    });
    expect(addButton.hasAttribute("disabled")).toBe(false);
    expect(addButton.dataset.disabled).toBeUndefined();

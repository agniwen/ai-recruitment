          ...
          <TooltipProvider>
            <MemoryRouter initialEntries={["/agent-builder/agents/agent-123/edit"]}>
              ...
              <Routes>
                ...
                <Route
                  path="/agent-builder/agents/:id/view"
                  element={<div data-testid="view-page" />}
                />
                <Route
                  path="/agent-builder/agents"
                  element={<div data-testid="agents-list-page" />}
                />
              </Routes>
            </MemoryRouter>
          </TooltipProvider>
...
const commonHandlers = (overrides?: {
  agent?: Partial<typeof storedAgent>;
  meDelay?: Promise<void>;
}) => [
  http.get(`${BASE_URL}/api/auth/capabilities`, () =>
    HttpResponse.json(authEnabledNoRbacCapabilities),
  ),
  http.get(`${BASE_URL}/api/auth/me`, async () => {
    if (overrides?.meDelay) {
      await overrides.meDelay;
    }
    return HttpResponse.json({ id: "user-1" });
  }),
  http.get(`${BASE_URL}/api/stored/agents/agent-123`, () =>
    HttpResponse.json({ ...storedAgent, ...overrides?.agent }),
  ),
...
describe("AgentBuilderAgentEdit — navigation, header, autosave", () => {
  ...
  it("redirects to the agents list when no stored agent exists (404)", async () => {
    server.use(
      ...
      http.get(
        `${BASE_URL}/api/stored/agents/agent-123`,
  ...
  it("autosaves edits made in the configure panel and persists them via PATCH", async () => {
    ...
    const lastBody = patchBodies.at(-1) as Record<string, unknown>;
    ...
    expect(screen.getByTestId("current-location").textContent).toBe(
      "/agent-builder/agents/agent-123/edit",
    );
  });
        ...
        agent: {
          toolProviders: {
            composio: {
              ...
              tools: {
                GMAIL_FETCH_EMAILS: { description: "Fetch emails from Gmail", toolkit: "gmail" },
              },
            },
          },
        },

...
import {
  ...
  memberDirectManagerInputSchema,
...
export const workspaceRouter = factory
  ...
  .get("/members/hierarchy", requirePermission("member", "update"), async (c) => {
    const { activeOrg } = c.var;
    if (!activeOrg) {
      return c.json({ message: "Unauthorized" }, 401);
    }
  ...
  .patch(
    ...
    requirePermission("member", "update"),
    zValidator("json", memberInterviewerInputSchema, jsonValidatorError("成员面试官参数无效。")),
    async (c) => {
      const { activeOrg } = c.var;
      if (!activeOrg) {
        return c.json({ message: "Unauthorized" }, 401);
      }
      const status = await updateWorkspaceMemberInterviewer({
        isInterviewer: c.req.valid("json").isInterviewer,
        organizationId: activeOrg.id,
        userId: c.req.param("userId"),
      });
      if (status === "missing") {

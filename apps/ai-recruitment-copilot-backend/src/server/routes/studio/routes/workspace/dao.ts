...
export async function listWorkspaceMemberHierarchy(
  ...
  const userIdByMemberId = new Map(rows.map((row) => [row.id, row.userId]));
  return rows.map((row) => ({
    directManagerUserId: row.directManagerId
      ? (userIdByMemberId.get(row.directManagerId) ?? null)
      : null,
    userId: row.userId,
  }));
}
...
export function updateWorkspaceMemberDirectManager({
  ...
  return db.transaction(async (tx) => {
    ...
    const target = memberByUserId.get(userId);
    if (!target) {
      return "missing";
    }
    ...
    const directManager = directManagerUserId ? memberByUserId.get(directManagerUserId) : undefined;
    if (directManagerUserId && !directManager) {
      return "missing";
    }

    const memberById = new Map(rows.map((row) => [row.id, row]));
    const visited = new Set<string>();
    let ancestor = directManager;
    while (ancestor) {
      if (ancestor.id === target.id || visited.has(ancestor.id)) {
        return "cycle";
      }
      visited.add(ancestor.id);
      ancestor = ancestor.directManagerId ? memberById.get(ancestor.directManagerId) : undefined;
    }

    await tx
      .update(member)
      .set({ directManagerId: directManager?.id ?? null })
      .where(and(eq(member.organizationId, organizationId), eq(member.id, target.id)));
    return "updated";
  });
}

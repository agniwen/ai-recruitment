import { and, asc, count, desc, eq, inArray, isNotNull, isNull, min, ne, sql } from "drizzle-orm";
...
function timestampConditions(column: SQLWrapper, range: InstantRange): (SQL | undefined)[] {
  return [
    range.start ? sql`${column} >= ${range.start}` : undefined,
    range.end ? sql`${column} < ${range.end}` : undefined,
  ];
}

function timestampCondition(column: SQLWrapper, range: InstantRange): SQL {
  return and(...timestampConditions(column, range)) ?? sql`true`;
}
...
async function loadCandidateMetrics(
  ...
  const [row] = await db
    .select({
      associated:
        sql<number>`COUNT(*) FILTER (WHERE ${timestampCondition(studioInterview.createdAt, range)})`.mapWith(
          Number,
        ),
      onboarded: sql<number>`COUNT(*) FILTER (WHERE ${and(
        eq(studioInterview.outcome, "hired"),
        isNotNull(studioInterview.actualOnboardedAt),
        ...timestampConditions(studioInterview.actualOnboardedAt, range),
      )})`.mapWith(Number),
      pendingEvaluation: sql<number>`COUNT(*) FILTER (WHERE ${and(
        ...
        ...timestampConditions(studioInterview.createdAt, range),
      )})`.mapWith(Number),
      rejected: sql<number>`COUNT(*) FILTER (WHERE ${and(
        eq(studioInterview.outcome, "rejected"),
        ...timestampConditions(studioInterview.closedAt, range),
      )})`.mapWith(Number),
      todayAssociated:
        sql<number>`COUNT(*) FILTER (WHERE ${timestampCondition(studioInterview.createdAt, todayRange)})`.mapWith(
          Number,
        ),
      todayOnboarded: sql<number>`COUNT(*) FILTER (WHERE ${and(
        eq(studioInterview.outcome, "hired"),
        isNotNull(studioInterview.actualOnboardedAt),
        ...timestampConditions(studioInterview.actualOnboardedAt, todayRange),
      )})`.mapWith(Number),
      todayPendingEvaluation: sql<number>`COUNT(*) FILTER (WHERE ${and(
        eq(studioInterview.pipelineStage, CURRENT_PENDING_EVALUATION_FACT.pipelineStage),
        eq(studioInterview.outcome, CURRENT_PENDING_EVALUATION_FACT.outcome),
        isNull(studioInterview.resumeEvaluationStatus),
        ...timestampConditions(studioInterview.createdAt, todayRange),
      )})`.mapWith(Number),
      todayRejected: sql<number>`COUNT(*) FILTER (WHERE ${and(
        eq(studioInterview.outcome, "rejected"),
        ...timestampConditions(studioInterview.closedAt, todayRange),
      )})`.mapWith(Number),
      ...
      .select({
        overall:
          sql<number>`COUNT(DISTINCT ${studioInterviewSchedule.interviewRecordId}) FILTER (WHERE ${and(
            ...
            ...timestampConditions(studioInterviewSchedule.scheduledAt, range),
          )})`.mapWith(Number),
        today:
          sql<number>`COUNT(DISTINCT ${studioInterviewSchedule.interviewRecordId}) FILTER (WHERE ${and(
            ne(studioInterviewSchedule.status, "cancelled"),
            ...timestampConditions(studioInterviewSchedule.scheduledAt, todayRange),
          )})`.mapWith(Number),
      })
    ...
    db
      ...
      .where(
        and(
          ...
          selectedJobCondition(jobIds),
          ne(studioInterviewSchedule.status, "cancelled"),
          ...timestampConditions(studioInterviewSchedule.scheduledAt, futureRange),
        ),
      ),
  ]);
  ...
  const [row] = await db
    .select({
      completed: sql<number>`COUNT(*) FILTER (WHERE ${and(
        ...
        ...timestampConditions(studioHumanInterviewRound.scheduledAt, todayRange),
      )})`.mapWith(Number),
      ...
      overall: sql<number>`COUNT(*) FILTER (WHERE ${and(
        ...
        ...timestampConditions(studioHumanInterviewRound.scheduledAt, range),
      )})`.mapWith(Number),
...
async function loadOfferMetrics(
  ...
  const [firstSentRows, offerRows] = await Promise.all([
    db
      .select({
        ...
        sentAt: min(studioOfferDraft.sentAt),
      })
      ...
      .groupBy(studioOfferDraft.interviewRecordId),
    db
      .select({
        interviewRecordId: studioOfferDraft.interviewRecordId,
  ...
  for (const offer of offerRows) {
    if (!latestOffers.has(offer.interviewRecordId)) {
  ...
  for (const offer of latestOffers.values()) {
    if (offer.status !== "accepted" || !offer.joiningDate) {
      continue;
    }
  ...
  return {
    ...
    expectedOverall,
    expectedToday,
    overall: countFirstSentOffers(sentOffers, range),
    today: countFirstSentOffers(sentOffers, todayRange),
  };
}
...
export async function loadOdcAnalysisData(
  ...
  const [candidate, ai, human, offer, demandOnboarded] = await Promise.all([
    loadCandidateMetrics(organizationId, jobIds, range, todayRange),
    loadAiMetrics(organizationId, jobIds, range, todayRange, futureRange, futureDays),
    loadHumanMetrics(organizationId, jobIds, range, todayRange),
    loadOfferMetrics(organizationId, jobIds, range, todayRange, futureDays),
...
export async function loadOdcAnalysis(
  organizationId: string,
  filters: OdcAnalysisFilters,
): Promise<{ data: OdcAnalysisData; jobs: OdcAnalysisJobOption[] }> {
